function(newDoc, oldDoc, userCtx) {
    if (newDoc.type === "event") {
        ...
        if (newDoc.repeats) {
            if (newDoc.repeats !== "monthly") {
                throw ({ forbidden: "unkown value for repeats: " + newDoc.repeats })
            }

            if (typeof (newDoc.repeatsUntil) !== "string" || newDoc.repeatsUntil.length !== 10 || newDoc.repeatsUntil.split("-").length !== 3) {
                throw ({ forbidden: "repeatsUntil must be a string with format YYYY-MM-DD" })
            }
        }
