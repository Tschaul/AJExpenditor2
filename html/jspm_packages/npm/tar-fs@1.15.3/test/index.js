...
test('follow symlinks', function(t) {
  ...
  rimraf.sync(b);
  tar.pack(a, {dereference: true}).pipe(tar.extract(b)).on('finish', function() {
    var files = fs.readdirSync(b).sort();
    t.same(files.length, 2);
    t.same(files[0], '.gitignore');
    t.same(files[1], 'link');
...
test('strip', function(t) {
  ...
  rimraf.sync(b);
  tar.pack(a).pipe(tar.extract(b, {strip: 1})).on('finish', function() {
    var files = fs.readdirSync(b).sort();
    t.same(files.length, 1);
...
test('map + dir + permissions', function(t) {
  ...
  tar.pack(a).pipe(tar.extract(b, {map: aWithMode})).on('finish', function() {
    var files = fs.readdirSync(b).sort();
...
test('specific entries', function(t) {
  ...
  rimraf.sync(b);
  tar.pack(a, {entries: entries}).pipe(tar.extract(b)).on('finish', function() {
    var files = fs.readdirSync(b);
...
test('check type while mapping header on packing', function(t) {
  ...
  var checkHeaderType = function(header) {
    if (header.name.indexOf('.') === -1)
      t.same(header.type, header.name);
  };
