...
eqArr = function(arr1, arr2, recMap) {
  ...
  if (length !== arr2.length)
    return false;
  for (i = 0; i < length; ++i) {
    if (objHasOwnProperty.call(arr1, i) !== objHasOwnProperty.call(arr2, i))
      return false;
    if (!eqVal(arr1[i], arr2[i], recMap))
      return false;
  }
  return true;
};
eqObj = function(obj1, obj2, recMap) {
  ...
  return k1.every(function(key) {
    if (!objPropertyIsEnumerable.call(obj2, key))
      return false;
...
eqVal = function(val1, val2, recMap) {
  ...
  if (i === -1) {
    i = c1.push(val1) - 1;
    c2[i] = [];
  } else if (c2[i].indexOf(val2) !== -1)
    return true;
  c2[i].push(val2);
  return eqX(val1, val2, recMap);
};
module.exports = function(val1, val2) {
  if (eq(value(val1), value(val2)))
    return true;
