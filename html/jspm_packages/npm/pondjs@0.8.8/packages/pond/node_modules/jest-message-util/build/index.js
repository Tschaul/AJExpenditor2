...
const formatStackTrace = (stack, config, options, testPath) => {
  ...
  const relativeTestPath = testPath ? slash(path.relative(config.rootDir, testPath)) : null;
  lines = removeInternalStackEntries(lines, options);
  return lines.map(trimPaths).map(formatPaths.bind(null, config, options, relativeTestPath)).map((line) => STACK_INDENT + line).join('\n');
};
