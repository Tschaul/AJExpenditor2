...
(function(process) {
  ...
  var _require = require('../../jest-message-util/build/index');
  const formatExecError = _require.formatExecError;
  const snapshot = require('../../jest-snapshot/build/index');
  const pify = require('../../pify/index');
  const throat = require('../../throat/index');
  ...
  class TestRunner {
    ...
    runTests(tests, watcher) {
      ...
      return _asyncToGenerator(function*() {
        ...
        const estimatedTime = Math.ceil(getEstimatedTime(timings, _this._options.maxWorkers) / 1000);
        const runInBand = _this._options.maxWorkers <= 1 || tests.length <= 1 || tests.length <= 20 && timings.length > 0 && timings.every(function(timing) {
        ...
        const onFailure = function(test, error) {
          ...
          testResult.failureMessage = formatExecError(testResult, test.context.config, _this._globalConfig, test.path);
          addResult(aggregatedResults, testResult);
          _this._dispatcher.onTestResult(test, testResult, aggregatedResults);
        };
        const updateSnapshotState = function() {
          contexts.forEach(function(context) {
            const status = snapshot.cleanup(context.hasteFS, _this._globalConfig.updateSnapshot);
        ...
        try {
          yield runInBand ? _this._createInBandTestRun(tests, watcher, onResult, onFailure) : _this._createParallelTestRun(tests, watcher, onResult, onFailure);
        } catch (error) {
          if (!watcher.isInterrupted()) {
            throw error;
          }
    ...
    _createInBandTestRun(tests, watcher, onResult, onFailure) {
      ...
      return tests.reduce((promise, test) => mutex(() => promise.then(() => {
        ...
        return runTest(test.path, this._globalConfig, test.context.config, test.context.resolver);
    ...
    _setupReporters() {
      ...
      if (collectCoverage) {
        ...
        this.addReporter(new CoverageReporter(this._globalConfig, {maxWorkers: this._options.maxWorkers}));
      }
      if (notify) {
    ...
    _setupDefaultReporters() {
      this.addReporter(this._globalConfig.verbose ? new VerboseReporter(this._globalConfig) : new DefaultReporter(this._globalConfig));
    ...
    _bailIfNeeded(contexts, aggregatedResults, watcher) {
      if (this._globalConfig.bail && aggregatedResults.numFailedTests !== 0) {
        if (watcher.isWatchMode()) {
          ...
          return this._dispatcher.onRunComplete(contexts, aggregatedResults).then(exit).catch(exit);
        }
      }
      return Promise.resolve();
    }
