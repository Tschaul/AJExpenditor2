...
(function(process) {
  ...
  const formatTestPathPattern = (pattern) => {
    ...
    const formattedInput = pattern.shouldTreatInputAsPattern ? `/${input || ''}/` : `"${input || ''}"`;
  ...
  const getNoTestsFoundMessage = (testRunData, pattern) => {
    if (pattern.onlyChanged) {
      return (chalk.bold('No tests found related to files changed since last commit.\n') + chalk.dim(pattern.watch ? 'Press `a` to run all tests, or run Jest with `--watchAll`.' : 'Run Jest without `-o` to run all tests.'));
    }
  ...
  const getTestPaths = (() => {
    var _ref = _asyncToGenerator(function*(globalConfig, context, pattern, argv, pipe) {
      ...
      if (!data.tests.length) {
        if (pattern.onlyChanged && data.noSCM) {
          if (globalConfig.watch) {
            ...
            new Console(pipe, pipe).log('Jest can only find uncommitted changed files in a git or hg ' + 'repository. If you make your project a git or hg ' + 'repository (`git init` or `hg init`), Jest will be able ' + 'to only run tests related to files changed since the last ' + 'commit.');
          }
        }
      }
      return data;
  ...
  const processResults = (runResults, options) => {
    ...
    if (options.isJSON) {
      if (options.outputFile) {
        ...
        fs.writeFileSync(outputFile, JSON.stringify(formatTestResults(runResults)));
        process.stdout.write(`Test results written to: ` + `${path.relative(process.cwd(), outputFile)}\n`);
      } else {
  ...
  const runJest = (() => {
    var _ref2 = _asyncToGenerator(function*(globalConfig, contexts, argv, pipe, testWatcher, startRun, onComplete) {
      ...
      const testRunData = yield Promise.all(contexts.map((() => {
        var _ref3 = _asyncToGenerator(function*(context) {
          const matches = yield getTestPaths(globalConfig, context, pattern, argv, pipe);
      ...
      if (argv.listTests) {
        console.log(JSON.stringify(allTests.map(function(test) {
          return test.path;
        })));
