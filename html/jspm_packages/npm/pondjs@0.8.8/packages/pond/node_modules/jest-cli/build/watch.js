...
(function(process) {
  ...
  const watch = function(initialGlobalConfig, contexts, argv, pipe, hasteMapInstances) {
    ...
    hasteMapInstances.forEach((hasteMapInstance, index) => {
      hasteMapInstance.on('change', (_ref) => {
        ...
        const validPaths = eventsQueue.filter((_ref2) => {
          ...
          return isValidPath(initialGlobalConfig, contexts[index].config, filePath);
        });
        if (validPaths.length) {
          const context = contexts[index] = createContext(contexts[index].config, {
            hasteFS,
            moduleMap
          });
          prompt.abort();
    ...
    const startRun = function() {
      let overrideConfig = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
      if (isRunning) {
        return null;
      }
      ...
      isRunning = true;
      const globalConfig = Object.freeze(Object.assign({}, initialGlobalConfig, overrideConfig, {
        testNamePattern: argv.testNamePattern,
        testPathPattern: argv.testPathPattern
      }));
      return runJest(globalConfig, contexts, argv, pipe, testWatcher, startRun, (results) => {
        isRunning = false;
    ...
    const onKeypress = (key) => {
      ...
      if (isRunning && testWatcher && [KEYS.Q, KEYS.ENTER, KEYS.A, KEYS.O, KEYS.P, KEYS.T].indexOf(key) !== -1) {
      ...
      switch (key) {
        ...
        case KEYS.P:
          testPathPatternPrompt.run((testPathPattern) => {
            ...
            startRun();
          }, onCancelPatternPrompt, {header: activeFilters(argv)});
          break;
        case KEYS.T:
  ...
  const activeFilters = function(argv) {
    ...
    if (testNamePattern || testPathPattern) {
      const filters = [testPathPattern ? chalk.dim('filename ') + chalk.yellow('/' + testPathPattern + '/') : null, testNamePattern ? chalk.dim('test name ') + chalk.yellow('/' + testNamePattern + '/') : null].filter((f) => !!f).join(', ');
