...
(function(process) {
  ...
  const DependencyResolver = require('../../jest-resolve-dependencies/build/index');
  const changedFiles = require('../../jest-changed-files/build/index');
  ...
  const determineSCM = (path) => Promise.all([git.isGitRepository(path), hg.isHGRepository(path)]);
  ...
  class SearchSource {
    constructor(context, options) {
      ...
      this._options = options || {skipNodeResolution: false};
      this._rootPattern = new RegExp(config.roots.map((dir) => escapePathForRegex(dir)).join('|'));
    ...
    findRelatedTests(allPaths) {
      const dependencyResolver = new DependencyResolver(this._context.resolver, this._context.hasteFS);
      return {tests: toTests(this._context, dependencyResolver.resolveInverse(allPaths, this.isTestFilePath.bind(this), {skipNodeResolution: this._options.skipNodeResolution}))};
    }
    ...
    findChangedTests(options) {
      return Promise.all(this._context.config.roots.map(determineSCM)).then((repos) => {
        if (!repos.every((_ref) => {
        ...
        return Promise.all(repos.map((_ref3) => {
          ...
          return [];
        })).then((changedPathSets) => this.findRelatedTests(new Set(Array.prototype.concat.apply([], changedPathSets))));
      });
    }
