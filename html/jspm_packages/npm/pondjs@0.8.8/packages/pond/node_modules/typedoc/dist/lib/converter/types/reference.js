...
var ReferenceConverter = (function(_super) {
  ...
  ReferenceConverter.prototype.convertNode = function(context, node, type) {
    ...
    if (node.typeArguments) {
      result.typeArguments = node.typeArguments.map(function(n) {
        return _this.owner.convertType(context, n);
      });
    }
    return result;
  };
  ReferenceConverter.prototype.convertType = function(context, type) {
    ...
    if (type.typeArguments) {
      result.typeArguments = type.typeArguments.map(function(t) {
        return _this.owner.convertType(context, null, t);
      });
    }
    return result;
  };
  ReferenceConverter.prototype.convertLiteral = function(context, symbol, node) {
    var _this = this;
    for (var _i = 0,
        _a = symbol.declarations; _i < _a.length; _i++) {
      var declaration_1 = _a[_i];
      if (context.visitStack.indexOf(declaration_1) !== -1) {
        if (declaration_1.kind === ts.SyntaxKind.TypeLiteral || declaration_1.kind === ts.SyntaxKind.ObjectLiteralExpression) {
...
ReferenceConverter = __decorate([components_1.Component({name: 'type:reference'})], ReferenceConverter);
