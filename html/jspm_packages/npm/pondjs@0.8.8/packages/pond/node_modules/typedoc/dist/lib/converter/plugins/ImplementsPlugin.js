...
var ImplementsPlugin = (function(_super) {
  ...
  ImplementsPlugin.prototype.analyzeClass = function(context, classReflection, interfaceReflection) {
    ...
    interfaceReflection.children.forEach(function(interfaceMember) {
      ...
      for (var index = 0,
          count = classReflection.children.length; index < count; index++) {
  ...
  ImplementsPlugin.prototype.copyComment = function(target, source) {
    if (target.comment && source.comment && target.comment.hasTag('inheritdoc')) {
      ...
      if (target instanceof index_1.SignatureReflection && target.parameters && source instanceof index_1.SignatureReflection && source.parameters) {
        for (var index = 0,
            count = target.parameters.length; index < count; index++) {
...
ImplementsPlugin = __decorate([components_1.Component({name: 'implements'})], ImplementsPlugin);
