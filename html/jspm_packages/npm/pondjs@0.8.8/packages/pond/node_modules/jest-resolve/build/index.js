...
(function(process) {
  ...
  const isBuiltinModule = require('../../is-builtin-module/index');
  ...
  class Resolver {
    ...
    resolveModule(from, moduleName, options) {
      ...
      if (hastePackage) {
        try {
          const module = path.join.apply(path, [path.dirname(hastePackage)].concat(parts));
        ...
        for (const _ref of moduleNameMapper) {
          ...
          if (regex.test(moduleName)) {
            ...
            if (!matches) {
              ...
              moduleName = mappedModuleName.replace(/\$([0-9]+)/g, (_, index) => matches[parseInt(index, 10)]);
            }
            return (this.getModule(moduleName) || Resolver.findNodeModule(moduleName, {
              basedir: dirname,
              browser: this._options.browser,
              extensions,
              moduleDirectory,
