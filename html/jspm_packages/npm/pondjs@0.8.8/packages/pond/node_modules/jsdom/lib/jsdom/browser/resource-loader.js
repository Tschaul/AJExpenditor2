...
(function(Buffer, process) {
  "use strict";
  const parseContentType = require('../../../../content-type-parser/lib/content-type-parser');
  const sniffHTMLEncoding = require('../../../../html-encoding-sniffer/lib/html-encoding-sniffer');
  ...
  exports.readFile = function(filePath, options, callback) {
    ...
    readableStream.on("end", () => {
      if (defaultEncoding) {
        const encoding = detectMetaCharset ? sniffHTMLEncoding(data, {defaultEncoding}) : whatwgEncoding.getBOMEncoding(data) || defaultEncoding;
  ...
  function readDataUrl(dataUrl, options, callback) {
    ...
    try {
      ...
      if (defaultEncoding) {
        ...
        const encoding = options.detectMetaCharset ? sniffHTMLEncoding(data.buffer, sniffOptions) : whatwgEncoding.getBOMEncoding(data.buffer) || whatwgEncoding.labelToName(contentType.get("charset")) || defaultEncoding;
    ...
    const req = request(url, requestOptions, (error, response, bufferData) => {
      if (!error) {
        if (defaultEncoding) {
          ...
          const encoding = detectMetaCharset ? sniffHTMLEncoding(bufferData, sniffOptions) : whatwgEncoding.getBOMEncoding(bufferData) || whatwgEncoding.labelToName(contentType.get("charset")) || defaultEncoding;
