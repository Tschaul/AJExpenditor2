...
(function(process) {
  ...
  var Rule = (function(_super) {
    ...
    Rule.prototype.apply = function(sourceFile) {
      var options = {destructuringAll: this.ruleArguments.length !== 0 && this.ruleArguments[0].destructuring === OPTION_DESTRUCTURING_ALL};
  ...
  var PreferConstWalker = (function(_super) {
      ...
      var cb = function(node) {
        ...
        if (boundary !== 0) {
          if (boundary === 1) {
            ...
            if (utils.isFunctionDeclaration(node) || utils.isMethodDeclaration(node) || utils.isFunctionExpression(node) || utils.isArrowFunction(node) || utils.isConstructorDeclaration(node)) {
            ...
            if ((utils.isForInStatement(node) || utils.isForOfStatement(node)) && node.initializer.kind !== ts.SyntaxKind.VariableDeclarationList) {
        ...
        } else if (utils.isPostfixUnaryExpression(node) || utils.isPrefixUnaryExpression(node) && (node.operator === ts.SyntaxKind.PlusPlusToken || node.operator === ts.SyntaxKind.MinusMinusToken)) {
    ...
    PreferConstWalker.prototype.handleBindingName = function(name, declarationInfo) {
      ...
      if (name.kind === ts.SyntaxKind.Identifier) {
        ...
        var destructuringInfo_1 = {reassignedSiblings: false};
    ...
    PreferConstWalker.prototype.handleVariableDeclaration = function(declarationList) {
      ...
      if (kind === 2 || utils.hasModifier(declarationList.parent.modifiers, ts.SyntaxKind.ExportKeyword, ts.SyntaxKind.DeclareKeyword)) {
        declarationInfo = {
          canBeConst: false,
          isBlockScoped: kind !== 0
        };
      } else {
        declarationInfo = {
          allInitialized: declarationList.parent.kind === ts.SyntaxKind.ForOfStatement || declarationList.parent.kind === ts.SyntaxKind.ForInStatement || declarationList.declarations.every(function(declaration) {
            return declaration.initializer !== undefined;
          }),
          canBeConst: true,
          declarationList: declarationList,
          isBlockScoped: kind === 1,
          isForLoop: declarationList.parent.kind === ts.SyntaxKind.ForStatement || declarationList.parent.kind === ts.SyntaxKind.ForOfStatement,
    ...
    PreferConstWalker.prototype.onScopeEnd = function(parent) {
      ...
      this.scope.variables.forEach(function(info, name) {
        if (info.declarationInfo.canBeConst && !info.reassigned && !(info.declarationInfo.reassignedSiblings && info.declarationInfo.isForLoop) && (!_this.options.destructuringAll || info.destructuringInfo === undefined || !info.destructuringInfo.reassignedSiblings)) {
          var fix = void 0;
          if (info.declarationInfo.allInitialized && !info.declarationInfo.reassignedSiblings && info.declarationInfo.isBlockScoped && !appliedFixes.has(info.declarationInfo.declarationList)) {
