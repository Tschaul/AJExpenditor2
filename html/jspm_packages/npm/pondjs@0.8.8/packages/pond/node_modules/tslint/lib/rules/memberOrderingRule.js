...
var allMemberKindNames = utils_1.mapDefined(Object.keys(MemberKind), function(key) {
  ...
  return typeof mk === "number" ? MemberKind[mk].replace(/[A-Z]/g, function(cap) {
...
var Rule = (function(_super) {
  ...
  Rule.metadata = {
    ...
    optionExamples: [[true, {order: "fields-first"}], [true, {order: ["static-field", "instance-field", "constructor", "public-instance-method", "protected-instance-method", "private-instance-method"]}], [true, {order: [{
        name: "static non-private",
...
var MemberOrderingWalker = (function(_super) {
  ...
  MemberOrderingWalker.prototype.checkMembers = function(members) {
    ...
    for (var _i = 0,
      ...
      if (rank < prevRank) {
        ...
        var errorLine1 = "Declaration of " + nodeType + " not allowed after declaration of " + prevNodeType + ". " + ("Instead, this should come " + locationHint + ".");
  ...
  MemberOrderingWalker.prototype.findLowerRank = function(members, targetRank) {
    var max = -1;
    for (var _i = 0,
        members_3 = members; _i < members_3.length; _i++) {
  ...
  MemberOrderingWalker.prototype.memberRank = function(member) {
    ...
    return this.options.order.findIndex(function(category) {
...
function getMemberKind(member) {
  var accessLevel = hasModifier(ts.SyntaxKind.PrivateKeyword) ? "private" : hasModifier(ts.SyntaxKind.ProtectedKeyword) ? "protected" : "public";
  switch (member.kind) {
...
function parseOptions(options) {
  var _a = getOptionsJson(options),
      orderJson = _a.order,
      alphabetize = _a.alphabetize;
  var order = orderJson.map(function(cat) {
    return typeof cat === "string" ? new MemberCategory(cat.replace(/-/g, " "), new Set(memberKindFromName(cat))) : new MemberCategory(cat.name, new Set(utils_1.flatMap(cat.kinds, memberKindFromName)));
  });
  return {
    order: order,
...
function convertFromOldStyleOptions(options) {
  var categories = [{
    name: "member",
    kinds: allMemberKindNames
  }];
  if (hasOption("variables-before-functions")) {
    categories = splitOldStyleOptions(categories, function(kind) {
      return kind.includes("field");
    }, "field", "method");
  }
  if (hasOption("static-before-instance")) {
    categories = splitOldStyleOptions(categories, function(kind) {
      return kind.includes("static");
    }, "static", "instance");
  }
  if (hasOption("public-before-private")) {
    categories = splitOldStyleOptions(categories, function(kind) {
      return !kind.includes("private");
    }, "public", "private");
  }
  return categories;
...
function splitOldStyleOptions(categories, filter, a, b) {
  ...
  for (var _i = 0,
      categories_1 = categories; _i < categories_1.length; _i++) {
