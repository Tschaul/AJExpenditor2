...
exports.Rule = Rule;
var TRANSFORMS = new Map([["any", function() {
  return "";
}], ["case-insensitive", function(x) {
  return x.toLowerCase();
}], ["lowercase-first", flipCase], ["lowercase-last", function(x) {
  return x;
}]]);
function parseOptions(ruleArguments) {
  var optionSet = ruleArguments[0];
  var _a = optionSet === undefined ? {} : optionSet,
      _b = _a["import-sources-order"],
      sources = _b === void 0 ? "case-insensitive" : _b,
      _c = _a["named-imports-order"],
      named = _c === void 0 ? "case-insensitive" : _c;
  return {
...
var Walker = (function(_super) {
  ...
  Walker.prototype.checkStatement = function(statement) {
    ...
    if (tsutils_1.isImportDeclaration(statement)) {
      ...
      if (body !== undefined) {
        for (var _i = 0,
            _a = body.statements; _i < _a.length; _i++) {
...
var ImportsBlock = (function() {
  ...
  ImportsBlock.prototype.getReplacement = function() {
    ...
    var sortedDeclarations = sortByKey(this.importDeclarations.slice(), function(x) {
      return x.sourcePath;
    });
    var fixedText = sortedDeclarations.map(function(x) {
      return x.text;
    }).join("");
