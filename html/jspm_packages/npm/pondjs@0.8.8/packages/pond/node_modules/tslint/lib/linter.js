...
var Linter = (function() {
  ...
  Linter.prototype.lint = function(fileName, source, configuration) {
    ...
    if (this.options.fix && fileFailures.some(function(f) {
      return f.hasFix();
    })) {
    ...
    for (var _i = 0,
        fileFailures_1 = fileFailures; _i < fileFailures_1.length; _i++) {
  ...
  Linter.prototype.getResult = function() {
    ...
    var errorCount = this.failures.filter(function(failure) {
      return failure.getRuleSeverity() === "error";
    }).length;
    return {
  ...
  Linter.prototype.getAllFailures = function(sourceFile, enabledRules) {
    var _this = this;
    var failures = utils_1.flatMap(enabledRules, function(rule) {
      return _this.applyRule(rule, sourceFile);
    });
  ...
  Linter.prototype.applyAllFixes = function(enabledRules, fileFailures, sourceFile, sourceFileName) {
    ...
    var _loop_1 = function(rule) {
      var hasFixes = fileFailures.some(function(f) {
        return f.hasFix() && f.getRuleName() === rule.getOptions().ruleName;
      });
      if (hasFixes) {
        var updatedFailures = enableDisableRules_1.removeDisabledFailures(sourceFile, this_1.applyRule(rule, sourceFile));
        var fixableFailures = updatedFailures.filter(function(f) {
          return f.hasFix();
        });
  ...
  Linter.prototype.applyFixes = function(sourceFilePath, source, fixableFailures) {
    var _this = this;
    var fixesByFile = createMultiMap(fixableFailures, function(f) {
      return [f.getFileName(), f.getFix()];
    });
