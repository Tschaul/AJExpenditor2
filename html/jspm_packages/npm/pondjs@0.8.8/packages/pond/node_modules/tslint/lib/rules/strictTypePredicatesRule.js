...
function walk(ctx, checker) {
  ...
  function checkEquals(node, _a) {
    ...
    switch (exprPred.kind) {
      case 0:
        {
          var predicate = exprPred.predicate,
              isNullOrUndefined = exprPred.isNullOrUndefined;
      ...
      case 1:
        {
          ...
          if (result !== undefined) {
            fail(typeof result === "boolean" ? Rule.FAILURE_STRING(result === isPositive) : Rule.FAILURE_STRICT_PREFER_STRICT_EQUALS(result, isPositive));
          }
        }
    }
...
function getTypePredicateOneWay(left, right, isStrictEquals) {
  switch (right.kind) {
    case ts.SyntaxKind.TypeOfExpression:
      ...
      return predicate === undefined ? {kind: 2} : {
        kind: 0,
        expression: expression,
        predicate: predicate,
        isNullOrUndefined: kind === "undefined"
      };
...
function getTypePredicateForKind(kind) {
  switch (kind) {
    ...
    case "function":
      return isFunction;
    case "object":
      var allFlags_1 = ts.TypeFlags.Undefined | ts.TypeFlags.Void | ts.TypeFlags.BooleanLike | ts.TypeFlags.NumberLike | ts.TypeFlags.StringLike | ts.TypeFlags.ESSymbol;
      return function(type) {
        return !Lint.isTypeFlagSet(type, allFlags_1) && !isFunction(type);
      };
    default:
      return undefined;
  }
...
function flagPredicate(testedFlag) {
  return function(type) {
    return Lint.isTypeFlagSet(type, testedFlag);
  };
}
...
function getConstantBoolean(type, predicate) {
  ...
  var anyFalse = false;
  for (var _i = 0,
      _a = unionParts(type); _i < _a.length; _i++) {
    var ty = _a[_i];
    if (predicate(ty)) {
      anyTrue = true;
    } else {
...
function testNonStrictNullUndefined(type) {
  ...
  return !anyOther ? true : anyNull && anyUndefined ? undefined : anyNull ? "null" : anyUndefined ? "undefined" : false;
}
