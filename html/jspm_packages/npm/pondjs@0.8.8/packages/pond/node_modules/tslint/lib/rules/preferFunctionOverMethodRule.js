...
var PreferFunctionOverMethodWalker = (function(_super) {
  ...
  PreferFunctionOverMethodWalker.prototype.walk = function(sourceFile) {
    ...
    var cb = function(node) {
      ...
      } else if (_this.currentScope !== undefined && (node.kind === ts.SyntaxKind.ThisKeyword && !isRecursiveCall(node, _this.currentScope.name) || node.kind === ts.SyntaxKind.SuperKeyword)) {
  ...
  PreferFunctionOverMethodWalker.prototype.isExempt = function(node) {
    return node.body === undefined || node.parent.kind !== ts.SyntaxKind.ClassDeclaration && node.parent.kind !== ts.SyntaxKind.ClassExpression || tsutils_1.hasModifier(node.modifiers, ts.SyntaxKind.StaticKeyword) || this.options.allowProtected && tsutils_1.hasModifier(node.modifiers, ts.SyntaxKind.ProtectedKeyword) || this.options.allowPublic && (tsutils_1.hasModifier(node.modifiers, ts.SyntaxKind.PublicKeyword) || !tsutils_1.hasModifier(node.modifiers, ts.SyntaxKind.ProtectedKeyword, ts.SyntaxKind.PrivateKeyword));
  };
  return PreferFunctionOverMethodWalker;
}(Lint.AbstractWalker));
function isRecursiveCall(node, name) {
  return name !== undefined && node.parent.kind === ts.SyntaxKind.PropertyAccessExpression && node.parent.name.text === name;
}
var _a;
