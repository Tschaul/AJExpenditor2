...
var SemicolonWalker = (function(_super) {
  ...
  SemicolonWalker.prototype.checkUnnecessary = function(node) {
    ...
    if (lastToken.kind === ts.SyntaxKind.YieldKeyword && lastToken.parent.kind === ts.SyntaxKind.YieldExpression || lastToken.kind === ts.SyntaxKind.CloseBraceToken && lastToken.parent.kind === ts.SyntaxKind.Block && lastToken.parent.parent.kind === ts.SyntaxKind.ArrowFunction) {
  ...
  SemicolonWalker.prototype.checkEmptyStatement = function(node) {
    if (!utils.isIterationStatement(node.parent)) {
      ...
      var noFix = parentKind === ts.SyntaxKind.IfStatement || parentKind === ts.SyntaxKind.LabeledStatement || parentKind === ts.SyntaxKind.WithStatement;
      this.reportUnnecessary(node.end, noFix);
    }
  };
  SemicolonWalker.prototype.visitPropertyDeclaration = function(node) {
    if (node.initializer !== undefined && node.initializer.kind === ts.SyntaxKind.ArrowFunction && !utils.isSameLine(this.sourceFile, node.getStart(this.sourceFile), node.end)) {
