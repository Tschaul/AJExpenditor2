...
var Rule = (function(_super) {
  ...
  Rule.metadata = {
    ...
    options: {
      ...
      items: {
        type: "string",
        enum: ["check-branch", "check-decl", "check-operator", "check-module", "check-separator", "check-type", "check-typecast", "check-preblock"]
      },
      minLength: 0,
      maxLength: 7
    },
...
function walk(ctx) {
  ...
  utils.forEachTokenWithTrivia(sourceFile, function(_text, tokenKind, range, parent) {
    if (tokenKind === ts.SyntaxKind.WhitespaceTrivia || tokenKind === ts.SyntaxKind.NewLineTrivia || tokenKind === ts.SyntaxKind.EndOfFileToken) {
    ...
    switch (tokenKind) {
      ...
      case ts.SyntaxKind.ImportKeyword:
        if (parent.kind === ts.SyntaxKind.CallExpression && parent.expression.kind === ts.SyntaxKind.ImportKeyword) {
          return;
        }
  ...
  function addMissingWhitespaceErrorAt(position) {
    if (ctx.failures.some(function(f) {
