...
var Rule = (function(_super) {
  ...
  Rule.metadata = {
    ...
    optionExamples: [[true, OPTION_USE_SPACES], [true, OPTION_USE_SPACES, OPTION_INDENT_SIZE_4], [true, OPTION_USE_TABS, OPTION_INDENT_SIZE_2]],
...
function walk(ctx) {
  ...
  for (var _i = 0,
      _b = tsutils_1.getLineRanges(sourceFile); _i < _b.length; _i++) {
    var _c = _b[_i],
        pos = _c.pos,
        contentLength = _c.contentLength;
    if (contentLength === 0) {
      continue;
    }
    ...
    if (token.kind !== ts.SyntaxKind.JsxText && (pos >= token.getStart(sourceFile) || tsutils_1.isPositionInComment(sourceFile, pos, token))) {
      continue;
    }
...
function createFix(lineStart, fullLeadingWhitespace, tabs, size) {
  ...
  var replaceRegExp = tabs ? new RegExp("^( {" + size + "})+( {1," + (size - 1) + "})?", "g") : /\t/g;
