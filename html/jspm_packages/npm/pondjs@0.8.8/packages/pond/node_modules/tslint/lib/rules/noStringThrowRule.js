  ...
  return ts.forEachChild(ctx.sourceFile, function cb(node) {
    if (tsutils_1.isThrowStatement(node)) {
      ...
      if (isString(expression)) {
        ctx.addFailureAtNode(node, Rule.FAILURE_STRING, [Lint.Replacement.appendText(expression.getStart(sourceFile), "new Error("), Lint.Replacement.appendText(expression.getEnd(), ")")]);
      }
    }
