        ...
        case "extra-parameter":
          {
            ...
            ctx.addFailureAtNode(extraParameter, extraParameter.dotDotDotToken !== undefined ? Rule.FAILURE_STRING_OMITTING_REST_PARAMETER(lineOfOtherOverload) : Rule.FAILURE_STRING_OMITTING_SINGLE_PARAMETER(lineOfOtherOverload));
          }
      }
    }
  }
...
function checkOverloads(signatures, typeParameters, getOverload) {
  ...
  for (var _i = 0,
      _a = collectOverloads(signatures, getOverload); _i < _a.length; _i++) {
    var overloads = _a[_i];
...
function compareSignatures(a, b, isTypeParameter) {
  ...
  return a.parameters.length === b.parameters.length ? signaturesDifferBySingleParameter(a.parameters, b.parameters) : signaturesDifferByOptionalOrRestParameter(a.parameters, b.parameters);
}
function signaturesCanBeUnified(a, b, isTypeParameter) {
  return typesAreEqual(a.type, b.type) && utils_1.arraysAreEqual(a.typeParameters, b.typeParameters, typeParametersAreEqual) && signatureUsesTypeParameter(a, isTypeParameter) === signatureUsesTypeParameter(b, isTypeParameter);
}
...
function getIsTypeParameter(typeParameters) {
  ...
  var set = new Set();
  for (var _i = 0,
      typeParameters_1 = typeParameters; _i < typeParameters_1.length; _i++) {
...
function signatureUsesTypeParameter(sig, isTypeParameter) {
  return sig.parameters.some(function(p) {
    return p.type !== undefined && typeContainsTypeParameter(p.type) === true;
  });
...
function collectOverloads(nodes, getOverload) {
  ...
  for (var _i = 0,
    ...
    var signature = overload.signature,
        key = overload.key;
...
function parametersHaveEqualSigils(a, b) {
  return (a.dotDotDotToken !== undefined) === (b.dotDotDotToken !== undefined) && (a.questionToken !== undefined) === (b.questionToken !== undefined);
}
