...
function walk(ctx) {
  ...
  for (var _i = 0,
      _a = tsutils_1.getLineRanges(sourceFile); _i < _a.length; _i++) {
    var line = _a[_i];
  ...
  var excludedRanges = ctx.options.ignoreTemplates ? ctx.options.ignoreJsDoc ? getExcludedRanges(sourceFile, ctx.options) : noConsecutiveBlankLinesRule_1.getTemplateRanges(sourceFile) : ctx.options.ignoreJsDoc ? getExcludedComments(sourceFile, ctx.options) : [];
  var _loop_1 = function(possibleFailure) {
    if (!excludedRanges.some(function(range) {
      return range.pos < possibleFailure.pos && possibleFailure.pos < range.end;
    })) {
...
function getExcludedComments(sourceFile, options) {
  ...
  tsutils_1.forEachComment(sourceFile, function(text, comment) {
    if (options.ignoreComments || options.ignoreJsDoc && isJsDoc(text, comment.kind, comment)) {
