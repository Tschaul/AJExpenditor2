...
var Rule = (function(_super) {
  ...
  Rule.defaultArguments = [exports.ARGUMENT_CLASSES, exports.ARGUMENT_FUNCTIONS, exports.ARGUMENT_METHODS, exports.ARGUMENT_PROPERTIES];
  Rule.ARGUMENT_DESCRIPTOR_BLOCK = {
    properties: (_a = {}, _a[exports.DESCRIPTOR_VISIBILITIES] = {
  ...
  Rule.ARGUMENT_DESCRIPTOR_CLASS = {
    properties: (_b = {}, _b[exports.DESCRIPTOR_LOCATIONS] = {
...
var Requirement = (function() {
  ...
  Requirement.constructRequirements = function(ruleArguments) {
    ...
    for (var _i = 0,
        ruleArguments_1 = ruleArguments; _i < ruleArguments_1.length; _i++) {
  ...
  Requirement.addRequirements = function(requirements, descriptor) {
    ...
    for (var type in descriptor) {
      if (utils_1.hasOwnProperty(descriptor, type)) {
        requirements.set(type, (type === "methods" || type === "properties") ? new ClassRequirement(descriptor[type]) : new BlockRequirement(descriptor[type]));
      }
    }
  };
...
var CompletedDocsWalker = (function(_super) {
  ...
  CompletedDocsWalker.prototype.checkComments = function(node, nodeDescriptor, comments, requirementNode) {
    if (comments.map(function(comment) {
      return comment.text;
    }).join("").trim() === "") {
  ...
  CompletedDocsWalker.prototype.describeDocumentationFailure = function(node, nodeType) {
    ...
    if (node.modifiers !== undefined) {
      description += node.modifiers.map(function(modifier) {
        return _this.describeModifier(modifier.kind);
      }).join(",") + " ";
    }
  ...
  CompletedDocsWalker.modifierAliases = {export: "exported"};
