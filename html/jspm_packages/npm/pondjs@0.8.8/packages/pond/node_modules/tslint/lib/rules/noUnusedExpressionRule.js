...
function isIndirectEval(node) {
  return tsutils_1.isIdentifier(node.right) && node.right.text === "eval" && isLiteralZero(node.left) && node.parent.kind === ts.SyntaxKind.ParenthesizedExpression && node.parent.parent.kind === ts.SyntaxKind.CallExpression;
}
