...
var Rule = (function(_super) {
  ...
  Rule.metadata = {
    ...
    options: {
      ...
      items: {
        type: "string",
        enum: [OPTION_ALLOW_NULL_UNION, OPTION_ALLOW_UNDEFINED_UNION, OPTION_ALLOW_STRING, OPTION_ALLOW_NUMBER, OPTION_ALLOW_BOOLEAN_OR_UNDEFINED]
      },
      minLength: 0,
      maxLength: 5
    },
    optionExamples: [true, [true, OPTION_ALLOW_NULL_UNION, OPTION_ALLOW_UNDEFINED_UNION, OPTION_ALLOW_STRING, OPTION_ALLOW_NUMBER], [true, OPTION_ALLOW_BOOLEAN_OR_UNDEFINED]],
    type: "functionality",
    typescriptOnly: true,
...
function walk(ctx, checker) {
  ...
  function checkExpression(node, location) {
    ...
    if (failure !== undefined) {
      if (failure === 0 && !options.strictNullChecks && (options.allowNullUnion || options.allowUndefinedUnion)) {
        return;
      }
...
function getTypeFailure(type, options) {
  ...
  var failure = failureForKind(kind, false, options);
  if (failure !== undefined) {
    return failure;
  }
  switch (triState(kind)) {
    case true:
      return Lint.isTypeFlagSet(type, ts.TypeFlags.Any | ts.TypeFlags.BooleanLiteral) ? undefined : 0;
    case false:
      return Lint.isTypeFlagSet(type, ts.TypeFlags.BooleanLiteral) ? undefined : 1;
    case undefined:
      return undefined;
  }
}
...
function handleUnion(type, options) {
  ...
  for (var _i = 0,
    ...
    var ty = _a[_i];
    var kind = getKind(ty);
    var failure = failureForKind(kind, true, options);
    if (failure !== undefined) {
      return failure;
    }
    switch (triState(kind)) {
  ...
  return seenFalsy === 0 ? 0 : !anyTruthy ? 1 : !options.allowMix && seenFalsy > 1 ? 7 : undefined;
}
function failureForKind(kind, isInUnion, options) {
  switch (kind) {
    case 0:
    case 1:
      return options.allowString ? undefined : 2;
    case 2:
    case 3:
      return options.allowNumber ? undefined : 3;
    case 8:
      return 6;
    case 6:
      return isInUnion && !options.allowNullUnion ? 4 : undefined;
    case 7:
      return isInUnion && !options.allowUndefinedUnion ? 5 : undefined;
    default:
      return undefined;
  }
}
...
function getKind(type) {
  return is(ts.TypeFlags.String) ? 0 : is(ts.TypeFlags.Number) ? 2 : is(ts.TypeFlags.Boolean) ? 4 : is(ts.TypeFlags.Null) ? 6 : is(ts.TypeFlags.Undefined | ts.TypeFlags.Void) ? 7 : is(ts.TypeFlags.EnumLike) ? 8 : is(ts.TypeFlags.NumberLiteral) ? (numberLiteralIsZero(type) ? 3 : 9) : is(ts.TypeFlags.StringLiteral) ? (stringLiteralIsEmpty(type) ? 1 : 9) : is(ts.TypeFlags.BooleanLiteral) ? (type.intrinsicName === "true" ? 9 : 5) : 9;
  function is(flags) {
...
function showFailure(location, ty, unionType, options) {
  ...
  var expected = expectedTypes.length === 1 ? "Only " + expectedTypes[0] + "s are allowed" : "Allowed types are " + stringOr(expectedTypes);
...
function showTypeFailure(ty, unionType, strictNullChecks) {
  ...
  switch (ty) {
    case 0:
      return strictNullChecks ? "is always truthy" : "is always truthy. It may be null/undefined, but neither " + ("'" + OPTION_ALLOW_NULL_UNION + "' nor '" + OPTION_ALLOW_UNDEFINED_UNION + "' is set");
    case 1:
