...
var Rule = (function(_super) {
  ...
  Rule.prototype.applyWithProgram = function(sourceFile, program) {
    ...
    }, {ignoreArrowFunctionShorthand: ignoreArrowFunctionShorthand});
  };
  Rule.metadata = {
...
function walk(ctx, checker) {
  var sourceFile = ctx.sourceFile,
      ignoreArrowFunctionShorthand = ctx.options.ignoreArrowFunctionShorthand;
  return ts.forEachChild(sourceFile, function cb(node) {
    if (isPossiblyVoidExpression(node) && !isParentAllowedVoid(node) && utils_1.isTypeFlagSet(checker.getTypeAtLocation(node), ts.TypeFlags.Void)) {
