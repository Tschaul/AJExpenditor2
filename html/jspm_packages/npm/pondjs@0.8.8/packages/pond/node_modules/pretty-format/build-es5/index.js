...
var style = require('../../ansi-styles/index');
...
var NEWLINE_REGEXP = /\n/gi;
var getSymbols = _getOwnPropertySymbols2.default || function(obj) {
  return [];
};
...
function printMap(val, indent, prevIndent, spacing, edgeSpacing, refs, maxDepth, currentDepth, plugins, min, callToJSON, printFunctionName, escapeRegex, colors) {
  ...
  if (!current.done) {
    ...
    while (!current.done) {
      ...
      var _value = print(current.value[1], indent, innerIndent, spacing, edgeSpacing, refs, maxDepth, currentDepth, plugins, min, callToJSON, printFunctionName, escapeRegex, colors);
...
function printObject(val, indent, prevIndent, spacing, edgeSpacing, refs, maxDepth, currentDepth, plugins, min, callToJSON, printFunctionName, escapeRegex, colors) {
  ...
  if (symbols.length) {
    keys = keys.filter(function(key) {
      return (!((typeof key === 'undefined' ? 'undefined' : (0, _typeof3.default)(key)) === 'symbol' || toString.call(key) === '[object Symbol]'));
    }).concat(symbols);
  }
  if (keys.length) {
    result += edgeSpacing;
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      var _value2 = print(val[key], indent, innerIndent, spacing, edgeSpacing, refs, maxDepth, currentDepth, plugins, min, callToJSON, printFunctionName, escapeRegex, colors);
...
function normalizeTheme(themeOption) {
  ...
  if ((typeof themeOption === 'undefined' ? 'undefined' : (0, _typeof3.default)(themeOption)) !== 'object') {
    throw new Error('pretty-format: Option "theme" must be of type "object" but instead received "' + (typeof themeOption === 'undefined' ? 'undefined' : (0, _typeof3.default)(themeOption)) + '".');
  }
