...
(function(process) {
  ...
  const jestPreset = require('../../babel-preset-jest/index');
  ...
  const createTransformer = (options) => {
    ...
    return {
      ...
      getCacheKey(fileData, filename, configString, _ref) {
        ...
        return crypto.createHash('md5').update(THIS_FILE).update('\0', 'utf8').update(fileData).update('\0', 'utf8').update(configString).update('\0', 'utf8').update(getBabelRC(filename)).update('\0', 'utf8').update(instrument ? 'instrument' : '').digest('hex');
      },
      process(src, filename, config, transformOptions) {
        if (!babel) {
          babel = require('../../babel-core/index');
        }
        ...
        if (transformOptions && transformOptions.instrument) {
          ...
          theseOptions.plugins = theseOptions.plugins.concat([[require('../../babel-plugin-istanbul/lib/index').default, {
            cwd: config.rootDir,
            exclude: []
          }]]);
        }
