...
(function() {
  var parsedTypeCheck,
      types,
      toString$ = {}.toString;
  parsedTypeCheck = require('../../type-check/lib/index').parsedTypeCheck;
  types = {
    '*': function(it) {
      switch (toString$.call(it).slice(8, -1)) {
        case 'Array':
          return coerceType(it, {type: 'Array'});
        case 'Object':
          return coerceType(it, {type: 'Object'});
        default:
          return {
            type: 'Just',
            value: coerceTypes(it, [{type: 'Undefined'}, {type: 'Null'}, {type: 'NaN'}, {type: 'Boolean'}, {type: 'Number'}, {type: 'Date'}, {type: 'RegExp'}, {type: 'Array'}, {type: 'Object'}, {type: 'String'}], {explicit: true})
          };
      }
    },
    ...
    Array: function(it) {
      return coerceArray(it, {of: [{type: '*'}]});
    },
    Object: function(it) {
      return coerceFields(it, {of: {}});
    },
    String: function(it) {
      var that;
  ...
  function coerceFields(node, type) {
    ...
    return {
      ...
      value: (function() {
        ...
        for (key in ref$ = node) {
          value = ref$[key];
          results$[key] = coerceTypes(value, typeOf[key] || [{type: '*'}]);
        }
        return results$;
      }())
    };
