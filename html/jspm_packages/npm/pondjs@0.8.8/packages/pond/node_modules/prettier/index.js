...
(function(Buffer, process) {
  ...
  function hasClosureCompilerTypeCastComment(text, node) {
    return (node.comments && node.comments.some((comment) => comment.leading && isBlockComment(comment) && comment.value.match(/^\*\s*@type\s*{[^}]+}\s*$/) && getNextNonSpaceNonCommentCharacter$1(text, comment) === "("));
  }
  ...
  function printString(raw, options, isDirectiveLiteral) {
    ...
    const enclosingQuote = options.parser === "json" ? double.quote : shouldUseAlternateQuote ? alternate.quote : preferred.quote;
  ...
  function printNumber(rawNumber) {
    return (rawNumber.toLowerCase().replace(/^([\d.]+e)(?:\+|(-))?0*(\d)/, "$1$2$3").replace(/^([\d.]+)e[+-]?0+$/, "$1").replace(/^\./, "0.").replace(/(\.\d+?)0+(?=e|$)/, "$1").replace(/\.(?=e|$)/, ""));
  }
  var util$3 = {
    getPrecedence,
    shouldFlatten,
  ...
  function handleConditionalExpressionComments(enclosingNode, precedingNode, followingNode, comment, text) {
    const isSameLineAsPrecedingNode = precedingNode && !util$2.hasNewlineInRange(text, locEnd(precedingNode), locStart(comment));
  ...
  function handleMethodNameComments(text, enclosingNode, precedingNode, comment) {
    if (enclosingNode && precedingNode && (enclosingNode.type === "Property" || enclosingNode.type === "MethodDefinition") && precedingNode.type === "Identifier" && enclosingNode.key === precedingNode && getNextNonSpaceNonCommentCharacter(text, precedingNode) !== ":") {
      addTrailingComment(precedingNode, comment);
      return true;
    }
    return false;
  ...
  function handleImportDeclarationComments(text, enclosingNode, precedingNode, comment) {
    if (precedingNode && enclosingNode && enclosingNode.type === "ImportDeclaration" && util$2.hasNewline(text, util$2.locEnd(comment))) {
      addTrailingComment(precedingNode, comment);
      return true;
    }
    return false;
  ...
  function printLeadingComment(commentPath, print, options) {
    ...
    if (isBlock) {
      return concat([contents, util$2.hasNewline(options.originalText, locEnd(comment)) ? hardline : " "]);
    }
  ...
  function printTrailingComment(commentPath, print, options) {
    ...
    const isBlock = util$2.isBlockComment(comment);
    if (util$2.hasNewline(options.originalText, locStart(comment), {backwards: true})) {
  ...
  FastPath$1.prototype.call = function call(callback) {
    const s = this.stack;
  ...
  FastPath$1.prototype.each = function each(callback) {
    const s = this.stack;
    const origLen = s.length;
    let value = s[origLen - 1];
    const argc = arguments.length;
  ...
  FastPath$1.prototype.map = function map(callback) {
    const s = this.stack;
    const origLen = s.length;
    let value = s[origLen - 1];
    const argc = arguments.length;
  ...
  FastPath$1.prototype.needsParens = function(options) {
    ...
    if ((parent.type === "ArrowFunctionExpression" && parent.body === node && node.type !== "SequenceExpression" && startsWithNoLookaheadToken$1(node, false)) || (parent.type === "ExpressionStatement" && startsWithNoLookaheadToken$1(node, true))) {
      return true;
    }
    switch (node.type) {
      case "CallExpression":
      ...
      case "MemberExpression":
        {
          return (parent.type === "MemberExpression" && parent.object === node && node.optional);
        }
      case "SpreadElement":
      case "SpreadProperty":
      ...
      case "TSParenthesizedType":
        {
          ...
          if ((parent.type === "TypeParameter" || parent.type === "VariableDeclarator" || parent.type === "TypeAnnotation" || parent.type === "GenericTypeAnnotation" || parent.type === "TSTypeReference") && (node.typeAnnotation.type === "TypeAnnotation" && node.typeAnnotation.typeAnnotation.type !== "TSFunctionType" && grandParent.type !== "TSTypeOperator")) {
            return false;
          }
      ...
      case "FunctionTypeAnnotation":
        return (parent.type === "UnionTypeAnnotation" || parent.type === "IntersectionTypeAnnotation" || parent.type === "ArrayTypeAnnotation");
      case "StringLiteral":
      case "NumericLiteral":
      case "Literal":
        if (typeof node.value === "string" && parent.type === "ExpressionStatement" && ((options.parser !== "typescript" && !parent.directive) || (options.parser === "typescript" && options.originalText.substr(util$6.locStart(node) - 1, 1) === "("))) {
          const grandParent = this.getParentNode(1);
  ...
  var re = new RegExp(ansiRegex$1().source);
  ...
  function parse(text, opts) {
    ...
    try {
      ...
      if (loc) {
        ...
        error.codeFrame = codeFrame.codeFrameColumns(text, loc, {highlightCode: true});
        error.message += "\n" + error.codeFrame;
        throw error;
      }
      throw error.stack;
  ...
  function printSubtree(subtreeParser, path, print, options) {
    ...
    next.options = Object.assign({}, options, next.options, {originalText: next.text});
      ...
      case "TemplateElement":
        {
          ...
          if (parentParent && parentParent.type === "TaggedTemplateExpression" && parent.quasis.length === 1 && ((parentParent.tag.type === "MemberExpression" && parentParent.tag.object.name === "graphql" && parentParent.tag.property.name === "experimental") || (parentParent.tag.type === "Identifier" && (parentParent.tag.name === "gql" || parentParent.tag.name === "graphql")))) {
            return {
              ...
              transformDoc: (doc) => concat$3([indent$3(concat$3([softline$2, stripTrailingHardline(doc)])), softline$2]),
      ...
      case "attribute":
        {
          if (/(^@)|(^v-)|:/.test(node.key) && !/^\w+$/.test(node.value)) {
            return {
              ...
              transformDoc: (doc) => {
                return concat$3([node.key, '="', util$7.hasNewlineInRange(node.value, 0, node.value.length) ? doc : docUtils$1.removeLines(doc), '"']);
              }
            };
          }
        }
  ...
  function transformCssDoc(quasisDoc, parent) {
    ...
    const expressionDocs = parentNode.expressions ? parent.path.map(parent.print, "expressions") : [];
  ...
  function stripTrailingHardline(doc) {
    if (doc.type === "concat" && doc.parts[0].type === "concat" && doc.parts[0].parts.length === 2 && doc.parts[0].parts[1].type === "concat" && doc.parts[0].parts[1].parts.length === 2 && doc.parts[0].parts[1].parts[0].hard && doc.parts[0].parts[1].parts[1].type === "break-parent") {
  ...
  function isStyledJsx(path) {
    ...
    const parent = path.getParentNode();
    const parentParent = path.getParentNode(1);
    return (parentParent && node.quasis && parent.type === "JSXExpressionContainer" && parentParent.type === "JSXElement" && parentParent.openingElement.name.name === "style" && parentParent.openingElement.attributes.some((attribute) => attribute.name.name === "jsx"));
  }
  function isStyledComponents(path) {
    ...
    switch (tag.type) {
      case "MemberExpression":
        return (isStyledIdentifier(tag.object) || (/^[A-Z]/.test(tag.object.name) && tag.property.name === "extend"));
      case "CallExpression":
        return (isStyledIdentifier(tag.callee) || (tag.callee.type === "MemberExpression" && tag.callee.object.type === "MemberExpression" && isStyledIdentifier(tag.callee.object.object)));
      case "Identifier":
        return tag.name === "css";
      default:
        return false;
  ...
  function genericPrint$1(path, options, print) {
    ...
    switch (n.kind) {
      case "Document":
        {
          return concat$4([join$3(concat$4([hardline$4, hardline$4]), path.map(print, "definitions")), hardline$4]);
        }
      case "OperationDefinition":
        {
          return concat$4([n.name === null ? "" : n.operation, n.name ? concat$4([" ", path.call(print, "name")]) : "", n.variableDefinitions && n.variableDefinitions.length ? group$2(concat$4(["(", indent$4(concat$4([softline$3, join$3(concat$4([ifBreak$2("", ", "), softline$3]), path.map(print, "variableDefinitions"))])), softline$3, ")"])) : "", printDirectives(path, print, n), n.selectionSet ? (n.name === null ? "" : " ") : "", path.call(print, "selectionSet")]);
        }
      case "FragmentDefinition":
        {
          return concat$4(["fragment ", path.call(print, "name"), " on ", path.call(print, "typeCondition"), printDirectives(path, print, n), " ", path.call(print, "selectionSet")]);
        }
      case "SelectionSet":
        {
          return concat$4(["{", indent$4(concat$4([hardline$4, join$3(hardline$4, path.map(print, "selections"))])), hardline$4, "}"]);
        }
      case "Field":
        {
          return group$2(concat$4([n.alias ? concat$4([path.call(print, "alias"), ": "]) : "", path.call(print, "name"), n.arguments.length > 0 ? group$2(concat$4(["(", indent$4(concat$4([softline$3, join$3(concat$4([ifBreak$2("", ", "), softline$3]), path.map(print, "arguments"))])), softline$3, ")"])) : "", printDirectives(path, print, n), n.selectionSet ? " " : "", path.call(print, "selectionSet")]));
        }
      case "Name":
        {
          return n.value;
      ...
      case "ListValue":
        {
          return group$2(concat$4(["[", indent$4(concat$4([softline$3, join$3(concat$4([ifBreak$2("", ", "), softline$3]), path.map(print, "values"))])), softline$3, "]"]));
        }
      case "ObjectValue":
        {
          return group$2(concat$4(["{", options.bracketSpacing && n.fields.length > 0 ? " " : "", indent$4(concat$4([softline$3, join$3(concat$4([ifBreak$2("", ", "), softline$3]), path.map(print, "fields"))])), softline$3, ifBreak$2("", options.bracketSpacing && n.fields.length > 0 ? " " : ""), "}"]));
        }
      case "ObjectField":
      case "Argument":
        {
          return concat$4([path.call(print, "name"), ": ", path.call(print, "value")]);
        }
      case "Directive":
        {
          return concat$4(["@", path.call(print, "name"), n.arguments.length > 0 ? group$2(concat$4(["(", indent$4(concat$4([softline$3, join$3(concat$4([ifBreak$2("", ", "), softline$3]), path.map(print, "arguments"))])), softline$3, ")"])) : ""]);
        }
      case "NamedType":
        {
      ...
      case "VariableDefinition":
        {
          return concat$4([path.call(print, "variable"), ": ", path.call(print, "type"), n.defaultValue ? concat$4([" = ", path.call(print, "defaultValue")]) : ""]);
        }
      ...
      case "ObjectTypeDefinition":
        {
          return concat$4(["type ", path.call(print, "name"), n.interfaces.length > 0 ? concat$4([" implements ", join$3(", ", path.map(print, "interfaces"))]) : "", printDirectives(path, print, n), " {", n.fields.length > 0 ? indent$4(concat$4([hardline$4, join$3(hardline$4, path.map(print, "fields"))])) : "", hardline$4, "}"]);
        }
      case "FieldDefinition":
        {
          return concat$4([path.call(print, "name"), n.arguments.length > 0 ? group$2(concat$4(["(", indent$4(concat$4([softline$3, join$3(concat$4([ifBreak$2("", ", "), softline$3]), path.map(print, "arguments"))])), softline$3, ")"])) : "", ": ", path.call(print, "type"), printDirectives(path, print, n)]);
        }
      case "DirectiveDefinition":
        {
          return concat$4(["directive ", "@", path.call(print, "name"), n.arguments.length > 0 ? group$2(concat$4(["(", indent$4(concat$4([softline$3, join$3(concat$4([ifBreak$2("", ", "), softline$3]), path.map(print, "arguments"))])), softline$3, ")"])) : "", concat$4([" on ", join$3(" | ", path.map(print, "locations"))])]);
        }
      case "EnumTypeDefinition":
        {
          return concat$4(["enum ", path.call(print, "name"), printDirectives(path, print, n), " {", n.values.length > 0 ? indent$4(concat$4([hardline$4, join$3(hardline$4, path.map(print, "values"))])) : "", hardline$4, "}"]);
        }
      case "EnumValueDefinition":
        {
          return concat$4([path.call(print, "name"), printDirectives(path, print, n)]);
        }
      case "InputValueDefinition":
        {
          return concat$4([path.call(print, "name"), ": ", path.call(print, "type"), n.defaultValue ? concat$4([" = ", path.call(print, "defaultValue")]) : "", printDirectives(path, print, n)]);
        }
      case "InputObjectTypeDefinition":
        {
          return concat$4(["input ", path.call(print, "name"), printDirectives(path, print, n), " {", n.fields.length > 0 ? indent$4(concat$4([hardline$4, join$3(hardline$4, path.map(print, "fields"))])) : "", hardline$4, "}"]);
        }
      case "SchemaDefinition":
        {
          return concat$4(["schema", printDirectives(path, print, n), " {", n.operationTypes.length > 0 ? indent$4(concat$4([hardline$4, join$3(hardline$4, path.map(print, "operationTypes"))])) : "", hardline$4, "}"]);
        }
      case "OperationTypeDefinition":
        {
          return concat$4([path.call(print, "operation"), ": ", path.call(print, "type")]);
        }
      case "InterfaceTypeDefinition":
        {
          return concat$4(["interface ", path.call(print, "name"), printDirectives(path, print, n), " {", n.fields.length > 0 ? indent$4(concat$4([hardline$4, join$3(hardline$4, path.map(print, "fields"))])) : "", hardline$4, "}"]);
        }
      case "FragmentSpread":
        {
          return concat$4(["...", path.call(print, "name"), printDirectives(path, print, n)]);
      ...
      case "InlineFragment":
        {
          return concat$4(["...", n.typeCondition ? concat$4([" on ", path.call(print, "typeCondition")]) : "", printDirectives(path, print, n), " ", path.call(print, "selectionSet")]);
        }
      case "UnionTypeDefinition":
        {
          return group$2(concat$4(["union ", path.call(print, "name"), " =", ifBreak$2("", " "), indent$4(concat$4([ifBreak$2(concat$4([line$2, "  "])), join$3(concat$4([line$2, "| "]), path.map(print, "types"))]))]));
        }
      case "ScalarTypeDefinition":
        {
          return concat$4(["scalar ", path.call(print, "name"), printDirectives(path, print, n)]);
        }
      case "NonNullType":
        {
  ...
  function printDirectives(path, print, n) {
    ...
    return concat$4([" ", group$2(indent$4(concat$4([softline$3, join$3(concat$4([ifBreak$2("", " "), softline$3]), path.map(print, "directives"))])))]);
  }
      ...
      case "tag":
        {
          ...
          return group$3(concat$5([hasNewline ? hardline$5 : "", "<", n.name, printAttributes(path, print), n.children.length ? ">" : selfClose, n.name.toLowerCase() === "html" ? concat$5([hardline$5, children]) : indent$5(children), n.children.length ? concat$5([softline$4, "</", n.name, ">"]) : hardline$5]));
        }
      case "comment":
        {
  ...
  function printAttributes(path, print) {
    const node = path.getValue();
    return concat$5([node.attributes.length ? " " : "", indent$5(join$4(line$3, path.map(print, "attributes")))]);
  }
  ...
  function genericPrint$3(path, options, print) {
    ...
    switch (n.type) {
      ...
      case "css-rule":
        {
          return concat$6([path.call(print, "selector"), n.important ? " !important" : "", n.nodes ? concat$6([" {", n.nodes.length > 0 ? indent$6(concat$6([hardline$6, printNodeSequence(path, options, print)])) : "", hardline$6, "}"]) : ";"]);
        }
      case "css-decl":
        {
          const isValueExtend = n.value.type === "value-root" && n.value.group.type === "value-value" && n.value.group.group.type === "value-func" && n.value.group.group.value === "extend";
          const isComposed = n.value.type === "value-root" && n.value.group.type === "value-value" && n.prop === "composes";
          return concat$6([n.raws.before.replace(/[\s;]/g, ""), n.prop, ":", isValueExtend ? "" : " ", isComposed ? removeLines$1(path.call(print, "value")) : path.call(print, "value"), n.important ? " !important" : "", n.nodes ? concat$6([" {", indent$6(concat$6([softline$5, printNodeSequence(path, options, print)])), softline$5, "}"]) : ";"]);
        }
      case "css-atrule":
        {
          const hasParams = n.params && !(n.params.type === "media-query-list" && n.params.value === "");
          return concat$6(["@", n.name, hasParams ? concat$6([" ", path.call(print, "params")]) : "", n.nodes ? concat$6([" {", indent$6(concat$6([n.nodes.length > 0 ? softline$5 : "", printNodeSequence(path, options, print)])), softline$5, "}"]) : ";"]);
        }
      case "css-import":
        {
          return concat$6(["@", n.name, " ", n.directives ? concat$6([n.directives, " "]) : "", adjustStrings(n.importPath, options), n.nodes.length > 0 ? concat$6([" {", indent$6(concat$6([softline$5, printNodeSequence(path, options, print)])), softline$5, "}"]) : ";"]);
        }
      ...
      case "media-type":
        {
          ...
          if (parent.type === "css-atrule" && parent.name.toLowerCase() === "charset") {
            return n.value;
          }
      ...
      case "selector-attribute":
        {
          return concat$6(["[", n.attribute, n.operator ? n.operator : "", n.value ? quoteAttributeValue(adjustStrings(n.value, options), options) : "", n.insensitive ? " i" : "", "]"]);
        }
      case "selector-combinator":
        {
          if (n.value === "+" || n.value === ">" || n.value === "~") {
            const parent = path.getParentNode();
            const leading = parent.type === "selector-selector" && parent.nodes[0] === n ? "" : line$4;
      ...
      case "selector-pseudo":
        {
          return concat$6([n.value, n.nodes && n.nodes.length > 0 ? concat$6(["(", join$5(", ", path.map(print, "nodes")), ")"]) : ""]);
        }
      ...
      case "value-paren_group":
        {
          ...
          if (isURLCall && (n.groups.length === 1 || (n.groups.length > 0 && n.groups[0].type === "value-comma_group" && n.groups[0].groups.length > 0 && n.groups[0].groups[0].type === "value-word" && n.groups[0].groups[0].value === "data"))) {
            return concat$6([n.open ? path.call(print, "open") : "", join$5(",", path.map(print, "groups")), n.close ? path.call(print, "close") : ""]);
          }
          if (!n.open) {
            const printed = path.map(print, "groups");
            const res = [];
          ...
          return group$4(concat$6([n.open ? path.call(print, "open") : "", indent$6(concat$6([softline$5, join$5(concat$6([",", line$4]), path.map(print, "groups"))])), softline$5, n.close ? path.call(print, "close") : ""]));
        }
      case "value-value":
        {
          return path.call(print, "group");
  ...
  function printNodeSequence(path, options, print) {
    ...
    path.map((pathChild) => {
      ...
      if (i !== node.nodes.length - 1) {
        if ((node.nodes[i + 1].type === "css-comment" && !util$9.hasNewline(options.originalText, util$9.locStart(node.nodes[i + 1]), {backwards: true})) || (node.nodes[i + 1].type === "css-atrule" && node.nodes[i + 1].name === "else" && node.nodes[i].type !== "css-comment")) {
          parts.push(" ");
        } else {
  ...
  const STRING_OR_NUMBER_REGEX = RegExp(`${STRING_REGEX.source}|(${NUMBER_REGEX.source})`, "g");
  ...
  function adjustNumbers(value) {
    return value.replace(STRING_OR_NUMBER_REGEX, (match, quote, number) => (number ? printNumber$1(number) : match));
  }
  function printNumber$1(rawNumber) {
    return (util$9.printNumber(rawNumber).replace(/\.0(?=$|e)/, ""));
  }
  ...
  function hasJsxIgnoreComment(path) {
    const node = path.getValue();
    const parent = path.getParentNode();
    if (!parent || !node || node.type !== "JSXElement" || parent.type !== "JSXElement") {
      return false;
    }
  ...
  function genericPrint(path, options, printPath, args) {
    ...
    const decorators = [];
    if (node.decorators && node.decorators.length > 0 && !util$5.getParentExportDeclaration(path)) {
    ...
    } else if (util$5.isExportDeclaration(node) && node.declaration && node.declaration.decorators) {
      path.each((decoratorPath) => {
  ...
  function genericPrintNoParens(path, options, print, args) {
    ...
    switch (n.type) {
      ...
      case "Program":
        ...
        parts.push(comments$3.printDanglingComments(path, options, true));
      ...
      case "ExpressionStatement":
        ...
        return concat$2([path.call(print, "expression"), semi]);
      case "ParenthesizedExpression":
        return concat$2(["(", path.call(print, "expression"), ")"]);
      case "AssignmentExpression":
        return printAssignment(n.left, path.call(print, "left"), concat$2([" ", n.operator]), n.right, path.call(print, "right"), options);
      case "BinaryExpression":
      case "LogicalExpression":
        {
          const parent = path.getParentNode();
          const parentParent = path.getParentNode(1);
          const isInsideParenthesis = n !== parent.body && (parent.type === "IfStatement" || parent.type === "WhileStatement" || parent.type === "DoWhileStatement");
          ...
          return group$1(concat$2([parts.length > 0 ? parts[0] : "", indent$2(rest)]));
        }
      case "AssignmentPattern":
        return concat$2([path.call(print, "left"), " = ", path.call(print, "right")]);
      case "TSTypeAssertionExpression":
        return concat$2(["<", path.call(print, "typeAnnotation"), ">", path.call(print, "expression")]);
      case "MemberExpression":
        {
          const parent = path.getParentNode();
          ...
          return concat$2([path.call(print, "object"), shouldInline ? printMemberLookup(path, options, print) : group$1(indent$2(concat$2([softline$1, printMemberLookup(path, options, print)])))]);
        }
      case "MetaProperty":
        return concat$2([path.call(print, "meta"), ".", path.call(print, "property")]);
      case "BindExpression":
        if (n.object) {
      ...
      case "Identifier":
        {
          ...
          return concat$2([n.name, printOptionalToken(path), n.typeAnnotation && !isFunctionDeclarationIdentifier ? ": " : "", path.call(print, "typeAnnotation")]);
        }
      case "SpreadElement":
      ...
      case "ObjectTypeSpreadProperty":
        return concat$2(["...", path.call(print, "argument"), n.typeAnnotation ? ": " : "", path.call(print, "typeAnnotation")]);
      ...
      case "ArrowFunctionExpression":
        {
          ...
          if (canPrintParamsWithoutParens(n)) {
            ...
            parts.push(group$1(concat$2([printFunctionParams(path, print, options, args && (args.expandLastArg || args.expandFirstArg), true), printReturnType(path, print)])));
          }
          parts.push(" =>");
          const body = path.call((bodyPath) => print(bodyPath, args), "body");
          if (!hasLeadingOwnLineComment(options.originalText, n.body) && (n.body.type === "ArrayExpression" || n.body.type === "ObjectExpression" || n.body.type === "BlockStatement" || n.body.type === "JSXElement" || isTemplateOnItsOwnLine(n.body, options.originalText) || n.body.type === "ArrowFunctionExpression")) {
          ...
          const shouldAddParens = n.body.type === "ConditionalExpression" && !util$5.startsWithNoLookaheadToken(n.body, false);
      ...
      case "ImportDeclaration":
        {
          ...
          if (n.specifiers && n.specifiers.length > 0) {
            ...
            parts.push(" from ");
          } else if ((n.importKind && n.importKind === "type") || /{\s*}/.test(options.originalText.slice(util$5.locStart(n), util$5.locStart(n.source)))) {
            parts.push("{} from ");
          }
      ...
      case "BlockStatement":
        {
          ...
          const parent = path.getParentNode();
          const parentParent = path.getParentNode(1);
          if (!hasContent && !hasDirectives && !n.comments && (parent.type === "ArrowFunctionExpression" || parent.type === "FunctionExpression" || parent.type === "FunctionDeclaration" || parent.type === "ObjectMethod" || parent.type === "ClassMethod" || parent.type === "ForStatement" || parent.type === "WhileStatement" || parent.type === "DoWhileStatement" || (parent.type === "CatchClause" && !parentParent.finalizer))) {
            return "{}";
          }
          parts.push("{");
          if (hasDirectives) {
      ...
      case "ReturnStatement":
        ...
        if (n.argument) {
          if (returnArgumentHasLeadingComment(options, n.argument)) {
            parts.push(concat$2([" (", indent$2(concat$2([softline$1, path.call(print, "argument")])), line$1, ")"]));
          } else if (n.argument.type === "LogicalExpression" || n.argument.type === "BinaryExpression" || n.argument.type === "SequenceExpression") {
            parts.push(group$1(concat$2([ifBreak$1(" (", " "), indent$2(concat$2([softline$1, path.call(print, "argument")])), softline$1, ifBreak$1(")")])));
          } else {
            parts.push(" ", path.call(print, "argument"));
          }
        }
      ...
      case "TSInterfaceDeclaration":
        ...
        if (n.heritage.length) {
          parts.push(group$1(indent$2(concat$2([softline$1, "extends ", indent$2(join$2(concat$2([",", line$1]), path.map(print, "heritage"))), " "]))));
        }
      ...
      case "TSTypeLiteral":
        {
          ...
          const shouldBreak = n.type === "TSInterfaceBody" || (n.type !== "ObjectPattern" && util$5.hasNewlineInRange(options.originalText, util$5.locStart(n), util$5.locEnd(n)));
          const separator = n.type === "TSInterfaceBody" || n.type === "TSTypeLiteral" ? ifBreak$1(semi, ";") : ",";
          const fields = [];
          ...
          const canHaveTrailingSeparator = !(lastElem && (lastElem.type === "RestProperty" || lastElem.type === "RestElement"));
          let content;
          if (props.length === 0 && !n.typeAnnotation) {
            if (!hasDanglingComments(n)) {
              return concat$2([leftBrace, rightBrace]);
            }
            content = group$1(concat$2([leftBrace, comments$3.printDanglingComments(path, options), softline$1, rightBrace, printOptionalToken(path)]));
          } else {
            content = concat$2([leftBrace, indent$2(concat$2([options.bracketSpacing ? line$1 : softline$1, concat$2(props)])), ifBreak$1(canHaveTrailingSeparator && (separator !== "," || shouldPrintComma(options)) ? separator : ""), concat$2([options.bracketSpacing ? line$1 : softline$1, rightBrace]), printOptionalToken(path), n.typeAnnotation ? ": " : "", path.call(print, "typeAnnotation")]);
          }
          const parentParentParent = path.getParentNode(2);
          if ((n.type === "ObjectPattern" && parent && shouldHugArguments(parent) && parent.params[0] === n) || (shouldHugType(n) && parentParentParent && shouldHugArguments(parentParentParent) && parentParentParent.params[0].typeAnnotation.typeAnnotation === n)) {
            return content;
          }
      ...
      case "ArrayPattern":
        if (n.elements.length === 0) {
          if (!hasDanglingComments(n)) {
            ...
            parts.push(group$1(concat$2(["[", comments$3.printDanglingComments(path, options), softline$1, "]"])));
          }
        } else {
          ...
          parts.push(group$1(concat$2(["[", indent$2(concat$2([softline$1, printArrayItems(path, options, "elements", print)])), needsForcedTrailingComma ? "," : "", ifBreak$1(canHaveTrailingComma && !needsForcedTrailingComma && shouldPrintComma(options) ? "," : ""), comments$3.printDanglingComments(path, options, true), softline$1, "]"])));
        }
      ...
      case "ConditionalExpression":
        {
          ...
          if (n.test.type === "JSXElement" || n.consequent.type === "JSXElement" || n.alternate.type === "JSXElement" || parent.type === "JSXExpressionContainer" || firstNonConditionalParent.type === "JSXExpressionContainer" || conditionalExpressionChainContainsJSX(lastConditionalParent)) {
            jsxMode = true;
            forceNoIndent = true;
            const wrap = (doc) => concat$2([ifBreak$1("(", ""), indent$2(concat$2([softline$1, doc])), softline$1, ifBreak$1(")", "")]);
            const shouldNotWrap = (node) => node.type === "ConditionalExpression" || node.type === "NullLiteral" || (node.type === "Literal" && node.value === null);
            parts.push(" ? ", shouldNotWrap(n.consequent) ? path.call(print, "consequent") : wrap(path.call(print, "consequent")), " : ", shouldNotWrap(n.alternate) ? path.call(print, "alternate") : wrap(path.call(print, "alternate")));
          } else {
          ...
          const maybeGroup = (doc) => jsxMode ? parent === firstNonConditionalParent ? group$1(doc) : doc : group$1(doc);
          return maybeGroup(concat$2([path.call(print, "test"), forceNoIndent ? concat$2(parts) : indent$2(concat$2(parts))]));
        }
      case "VariableDeclaration":
        {
          ...
          parts = [isNodeStartingWithDeclare(n, options) ? "declare " : "", n.kind, firstVariable ? concat$2([" ", firstVariable]) : "", indent$2(concat$2(printed.slice(1).map((p) => concat$2([",", hasValue && !isParentForLoop ? hardline$2 : line$1, p]))))];
      ...
      case "VariableDeclarator":
        return printAssignment(n.id, concat$2([path.call(print, "id"), path.call(print, "typeParameters")]), " =", n.init, n.init && path.call(print, "init"), options);
      case "WithStatement":
        return group$1(concat$2(["with (", path.call(print, "object"), ")", adjustClause(n.body, path.call(print, "body"))]));
      case "IfStatement":
        {
          const con = adjustClause(n.consequent, path.call(print, "consequent"));
          const opening = group$1(concat$2(["if (", group$1(concat$2([indent$2(concat$2([softline$1, path.call(print, "test")])), softline$1])), ")", con]));
          parts.push(opening);
          if (n.alternate) {
      ...
      case "ForStatement":
        {
          ...
          const dangling = comments$3.printDanglingComments(path, options, true);
          ...
          return concat$2([printedComments, group$1(concat$2(["for (", group$1(concat$2([indent$2(concat$2([softline$1, path.call(print, "init"), ";", line$1, path.call(print, "test"), ";", line$1, path.call(print, "update")])), softline$1])), ")", body]))]);
        }
      case "WhileStatement":
        return group$1(concat$2(["while (", group$1(concat$2([indent$2(concat$2([softline$1, path.call(print, "test")])), softline$1])), ")", adjustClause(n.body, path.call(print, "body"))]));
      case "ForInStatement":
        return group$1(concat$2([n.each ? "for each (" : "for (", path.call(print, "left"), " in ", path.call(print, "right"), ")", adjustClause(n.body, path.call(print, "body"))]));
      case "ForOfStatement":
      case "ForAwaitStatement":
        {
          const isAwait = n.type === "ForAwaitStatement" || n.await;
          return group$1(concat$2(["for", isAwait ? " await" : "", " (", path.call(print, "left"), " of ", path.call(print, "right"), ")", adjustClause(n.body, path.call(print, "body"))]));
        }
      case "DoWhileStatement":
        {
          ...
          parts.push("while (");
          parts.push(group$1(concat$2([indent$2(concat$2([softline$1, path.call(print, "test")])), softline$1])), ")", semi);
          return concat$2(parts);
        }
      case "DoExpression":
      ...
      case "LabeledStatement":
        ...
        return concat$2([path.call(print, "label"), ": ", path.call(print, "body")]);
      case "TryStatement":
        return concat$2(["try ", path.call(print, "block"), n.handler ? concat$2([" ", path.call(print, "handler")]) : "", n.finalizer ? concat$2([" finally ", path.call(print, "finalizer")]) : ""]);
      case "CatchClause":
        return concat$2(["catch ", n.param ? concat$2(["(", path.call(print, "param"), ") "]) : "", path.call(print, "body")]);
      case "ThrowStatement":
        return concat$2(["throw ", path.call(print, "argument"), semi]);
      case "SwitchStatement":
        return concat$2(["switch (", path.call(print, "discriminant"), ") {", n.cases.length > 0 ? indent$2(concat$2([hardline$2, join$2(hardline$2, path.map((casePath) => {
          const caseNode = casePath.getValue();
          return concat$2([casePath.call(print), n.cases.indexOf(caseNode) !== n.cases.length - 1 && util$5.isNextLineEmpty(options.originalText, caseNode) ? hardline$2 : ""]);
        }, "cases"))])) : "", hardline$2, "}"]);
      case "SwitchCase":
        {
          if (n.test) {
      ...
      case "JSXExpressionContainer":
        {
          const parent = path.getParentNode(0);
          const shouldInline = n.expression.type === "ArrayExpression" || n.expression.type === "ObjectExpression" || n.expression.type === "ArrowFunctionExpression" || n.expression.type === "CallExpression" || n.expression.type === "FunctionExpression" || n.expression.type === "JSXEmptyExpression" || n.expression.type === "TemplateLiteral" || n.expression.type === "TaggedTemplateExpression" || (parent.type === "JSXElement" && (n.expression.type === "ConditionalExpression" || isBinaryish(n.expression)));
          if (shouldInline) {
            return group$1(concat$2(["{", path.call(print, "expression"), lineSuffixBoundary$1, "}"]));
          }
          return group$1(concat$2(["{", indent$2(concat$2([softline$1, path.call(print, "expression")])), softline$1, lineSuffixBoundary$1, "}"]));
        }
      case "JSXElement":
        {
      ...
      case "JSXOpeningElement":
        {
          const n = path.getValue();
          if (n.attributes.length === 1 && n.attributes[0].value && isStringLiteral(n.attributes[0].value)) {
            return group$1(concat$2(["<", path.call(print, "name"), " ", concat$2(path.map(print, "attributes")), n.selfClosing ? " />" : ">"]));
          }
          return group$1(concat$2(["<", path.call(print, "name"), concat$2([indent$2(concat$2(path.map((attr) => concat$2([line$1, print(attr)]), "attributes"))), n.selfClosing ? line$1 : options.jsxBracketSameLine ? ">" : softline$1]), n.selfClosing ? "/>" : options.jsxBracketSameLine ? "" : ">"]));
        }
      ...
      case "JSXEmptyExpression":
        {
          ...
          return concat$2([comments$3.printDanglingComments(path, options, !requiresHardline), requiresHardline ? hardline$2 : ""]);
        }
      case "ClassBody":
        if (!n.comments && n.body.length === 0) {
          return "{}";
        }
        return concat$2(["{", n.body.length > 0 ? indent$2(concat$2([hardline$2, path.call((bodyPath) => {
          return printStatementSequence(bodyPath, options, print);
        }, "body")])) : comments$3.printDanglingComments(path, options), hardline$2, "}"]);
      case "ClassProperty":
      case "TSAbstractClassProperty":
        {
          ...
          if (n.value) {
            parts.push(" =", printAssignmentRight(n.value, path.call(print, "value"), false, options));
          }
          parts.push(semi);
          return concat$2(parts);
        }
      ...
      case "TemplateLiteral":
        {
          ...
          path.each((childPath) => {
            ...
            if (i < expressions.length) {
              ...
              if (index !== -1) {
                size = util$5.getAlignmentSize(value.slice(index + 1).match(/^[ \t]*/)[0], tabWidth);
              }
      ...
      case "TupleTypeAnnotation":
        {
          ...
          return group$1(concat$2(["[", indent$2(concat$2([softline$1, printArrayItems(path, options, typesField, print)])), n.type === "TSTupleType" ? "" : ifBreak$1(shouldPrintComma(options) ? "," : ""), comments$3.printDanglingComments(path, options, true), softline$1, "]"]));
        }
      ...
      case "DeclareFunction":
        if (n.params) {
          return concat$2(["declare ", printFunctionDeclaration(path, print, options), semi]);
        }
      ...
      case "TSFunctionType":
        {
          ...
          const parentParent = path.getParentNode(1);
          const parentParentParent = path.getParentNode(2);
          let isArrowFunctionTypeAnnotation = n.type === "TSFunctionType" || !((parent.type === "ObjectTypeProperty" && !getFlowVariance(parent) && !parent.optional && util$5.locStart(parent) === util$5.locStart(n)) || parent.type === "ObjectTypeCallProperty" || (parentParentParent && parentParentParent.type === "DeclareFunction"));
          let needsColon = isArrowFunctionTypeAnnotation && parent.type === "TypeAnnotation";
          const needsParens = needsColon && isArrowFunctionTypeAnnotation && parent.type === "TypeAnnotation" && parentParent.type === "ArrowFunctionExpression";
          ...
          parts.push(printFunctionParams(path, print, options, false, true));
      ...
      case "FunctionTypeParam":
        return concat$2([path.call(print, "name"), printOptionalToken(path), n.name ? ": " : "", path.call(print, "typeAnnotation")]);
      case "GenericTypeAnnotation":
        return concat$2([path.call(print, "id"), path.call(print, "typeParameters")]);
      ...
      case "UnionTypeAnnotation":
        {
          const parent = path.getParentNode();
          const shouldIndent = parent.type !== "TypeParameterInstantiation" && parent.type !== "GenericTypeAnnotation" && parent.type !== "TSTypeReference" && !((parent.type === "TypeAlias" || parent.type === "VariableDeclarator") && hasLeadingOwnLineComment(options.originalText, n));
          ...
          const code = concat$2([ifBreak$1(concat$2([shouldIndent ? line$1 : "", "| "])), join$2(concat$2([line$1, "| "]), printed)]);
      ...
      case "ObjectTypeIndexer":
        {
          const variance = getFlowVariance(n);
          return concat$2([variance || "", "[", path.call(print, "id"), n.id ? ": " : "", path.call(print, "key"), "]: ", path.call(print, "value")]);
        }
      case "ObjectTypeProperty":
        {
          const variance = getFlowVariance(n);
          return concat$2([n.static ? "static " : "", isGetterOrSetter(n) ? n.kind + " " : "", variance || "", printPropertyKey(path, options, print), printOptionalToken(path), isFunctionNotation(n) ? "" : ": ", path.call(print, "value")]);
        }
      case "QualifiedTypeIdentifier":
        return concat$2([path.call(print, "qualification"), ".", path.call(print, "id")]);
      ...
      case "TypeCastExpression":
        return concat$2(["(", path.call(print, "expression"), ": ", path.call(print, "typeAnnotation"), ")"]);
      ...
      case "TSVoidKeyword":
        return "void";
      case "TSAsExpression":
        return concat$2([path.call(print, "expression"), " as ", path.call(print, "typeAnnotation")]);
      case "TSArrayType":
        return concat$2([path.call(print, "elementType"), "[]"]);
      ...
      case "TSTypeReference":
        return concat$2([path.call(print, "typeName"), printTypeParameters(path, options, print, "typeParameters")]);
      case "TSTypeQuery":
      ...
      case "TSIndexSignature":
        {
          const parent = path.getParentNode();
          return concat$2([n.export ? "export " : "", n.accessibility ? concat$2([n.accessibility, " "]) : "", n.static ? "static " : "", n.readonly ? "readonly " : "", "[", path.call(print, "index"), "]: ", path.call(print, "typeAnnotation"), parent.type === "ClassBody" ? semi : ""]);
        }
      case "TSTypePredicate":
        return concat$2([path.call(print, "parameterName"), " is ", path.call(print, "typeAnnotation")]);
      case "TSNonNullExpression":
        return concat$2([path.call(print, "expression"), "!"]);
      case "TSThisType":
        return "this";
      ...
      case "TSIndexedAccessType":
        return concat$2([path.call(print, "objectType"), "[", path.call(print, "indexType"), "]"]);
      ...
      case "TSCallSignature":
        {
          ...
          parts.push(group$1(printFunctionParams(path, print, options, false, true)));
          if (n.typeAnnotation) {
      ...
      case "TSMethodSignature":
        parts.push(n.accessibility ? concat$2([n.accessibility, " "]) : "", n.export ? "export " : "", n.static ? "static " : "", n.readonly ? "readonly " : "", n.computed ? "[" : "", path.call(print, "key"), n.computed ? "]" : "", printOptionalToken(path), printFunctionParams(path, print, options, false, true));
        if (n.typeAnnotation) {
          parts.push(": ", path.call(print, "typeAnnotation"));
        }
        return group$1(concat$2(parts));
      ...
      case "TSEnumDeclaration":
        ...
        if (n.members.length === 0) {
          ...
          parts.push(group$1(concat$2(["{", indent$2(concat$2([hardline$2, printArrayItems(path, options, "members", print), shouldPrintComma(options, "es5") ? "," : ""])), comments$3.printDanglingComments(path, options, true), hardline$2, "}"])));
        }
        return concat$2(parts);
      case "TSEnumMember":
        parts.push(path.call(print, "id"));
      ...
      case "TSModuleDeclaration":
        {
          ...
          if (parentIsDeclaration) {
            ...
            parts.push(printTypeScriptModifiers(path, options, print));
            const isGlobalDeclaration = n.id.type === "Identifier" && n.id.name === "global" && !/namespace|module/.test(options.originalText.slice(util$5.locStart(n), util$5.locStart(n.id)));
          ...
          if (bodyIsDeclaration) {
            ...
            parts.push(" {", indent$2(concat$2([line$1, path.call((bodyPath) => comments$3.printDanglingComments(bodyPath, options, true), "body"), group$1(path.call(print, "body"))])), line$1, "}");
          } else {
            parts.push(semi);
          }
          return concat$2(parts);
  ...
  function printPropertyKey(path, options, print) {
    ...
    const key = node.key;
    if (isStringLiteral(key) && isIdentifierName(key.value) && !node.computed && options.parser !== "json") {
  ...
  function printMethod(path, options, print) {
    ...
    parts.push(key, concat$2(path.call((valuePath) => [printFunctionTypeParameters(valuePath, options, print), group$1(concat$2([printFunctionParams(valuePath, print, options), printReturnType(valuePath, print)]))], "value")));
  ...
  function couldGroupArg(arg) {
    return ((arg.type === "ObjectExpression" && arg.properties.length > 0) || (arg.type === "ArrayExpression" && arg.elements.length > 0) || arg.type === "TSTypeAssertionExpression" || arg.type === "TSAsExpression" || arg.type === "FunctionExpression" || (arg.type === "ArrowFunctionExpression" && (arg.body.type === "BlockStatement" || arg.body.type === "ArrowFunctionExpression" || arg.body.type === "ObjectExpression" || arg.body.type === "ArrayExpression" || arg.body.type === "CallExpression" || arg.body.type === "JSXElement")));
  }
  function shouldGroupLastArg(args) {
    const lastArg = util$5.getLast(args);
    const penultimateArg = util$5.getPenultimate(args);
    return ((!lastArg.comments || !lastArg.comments.length) && couldGroupArg(lastArg) && (!penultimateArg || penultimateArg.type !== lastArg.type));
  }
  ...
  function printFunctionParams(path, print, options, expandArg, printTypeParams) {
    ...
    if (printed.length === 0) {
      return concat$2([typeParams, "(", comments$3.printDanglingComments(path, options, true), ")"]);
    }
    const lastParam = util$5.getLast(fun[paramsField]);
    if (expandArg && !(fun[paramsField] && fun[paramsField].some((n) => n.comments))) {
  ...
  function canPrintParamsWithoutParens(node) {
    return (node.params.length === 1 && !node.rest && !node.typeParameters && node.params[0].type === "Identifier" && !node.params[0].typeAnnotation && !node.params[0].comments && !node.params[0].optional && !node.predicate && !node.returnType);
  }
  function printFunctionDeclaration(path, print, options) {
    ...
    parts.push(printFunctionTypeParameters(path, options, print), group$1(concat$2([printFunctionParams(path, print, options), printReturnType(path, print)])), n.body ? " " : "", path.call(print, "body"));
    return concat$2(parts);
  }
  function printObjectMethod(path, options, print) {
    ...
    parts.push(printFunctionTypeParameters(path, options, print), group$1(concat$2([printFunctionParams(path, print, options), printReturnType(path, print)])), " ", path.call(print, "body"));
    return concat$2(parts);
  }
  ...
  function printExportDeclaration(path, options, print) {
    ...
    if (decl.declaration) {
      ...
      if (decl.specifiers && decl.specifiers.length > 0) {
        ...
        const isNamespaceFollowed = namespaceSpecifiers.length !== 0 && (specifiers.length !== 0 || defaultSpecifiers.length !== 0);
  ...
  function printTypeParameters(path, options, print, paramsKey) {
    ...
    const shouldInline = n[paramsKey].length === 1 && (shouldHugType(n[paramsKey][0]) || (n[paramsKey][0].type === "GenericTypeAnnotation" && shouldHugType(n[paramsKey][0].id)) || (n[paramsKey][0].type === "TSTypeReference" && shouldHugType(n[paramsKey][0].typeName)) || n[paramsKey][0].type === "NullableTypeAnnotation");
    if (shouldInline) {
      return concat$2(["<", join$2(", ", path.map(print, paramsKey)), ">"]);
    }
    return group$1(concat$2(["<", indent$2(concat$2([softline$1, join$2(concat$2([",", line$1]), path.map(print, paramsKey))])), ifBreak$1(options.parser !== "typescript" && shouldPrintComma(options, "all") ? "," : ""), softline$1, ">"]));
  }
  function printClass(path, options, print) {
    ...
    if (n.superClass) {
      ...
      const printed = concat$2(["extends ", path.call(print, "superClass"), path.call(print, "superTypeParameters")]);
      parts.push(path.call((superClass) => comments$3.printComments(superClass, () => printed, options), "superClass"));
    ...
    if (n.body && n.body.comments && hasLeadingOwnLineComment(options.originalText, n.body)) {
      parts.push(hardline$2);
    } else {
      parts.push(" ");
    }
  ...
  function printOptionalToken(path) {
    ...
    if (node.type === "CallExpression" || (node.type === "MemberExpression" && node.computed)) {
      return "?.";
    }
    return "?";
  }
  function printMemberLookup(path, options, print) {
    ...
    return group$1(concat$2([optional, "[", indent$2(concat$2([softline$1, property])), softline$1, "]"]));
  }
  ...
  function printMemberChain(path, options, print) {
    ...
    function rec(path) {
      ...
      if (node.type === "CallExpression" && isMemberish(node.callee)) {
        printedNodes.unshift({
          node: node,
          printed: comments$3.printComments(path, () => concat$2([printOptionalToken(path), printFunctionTypeParameters(path, options, print), printArgumentsList(path, options, print)]), options)
        });
        path.call((callee) => rec(callee), "callee");
      } else if (isMemberish(node)) {
        printedNodes.unshift({
          node: node,
          printed: comments$3.printComments(path, () => node.type === "MemberExpression" ? printMemberLookup(path, options, print) : printBindExpressionCallee(path, options, print), options)
        });
    ...
    printedNodes.unshift({
      node,
      printed: concat$2([printOptionalToken(path), printFunctionTypeParameters(path, options, print), printArgumentsList(path, options, print)])
    });
    path.call((callee) => rec(callee), "callee");
    const groups = [];
    ...
    const shouldMerge = groups.length >= 2 && !groups[1][0].node.comments && groups[0].length === 1 && (groups[0][0].node.type === "ThisExpression" || (groups[0][0].node.type === "Identifier" && (groups[0][0].node.name.match(/(^[A-Z])|^[_$]+$/) || (groups[1].length && groups[1][0].node.computed))));
    ...
    const flatGroups = groups.slice(0, cutoff).reduce((res, group) => res.concat(group), []);
    const hasComment = flatGroups.slice(1, -1).some((node) => hasLeadingComment(node.node)) || flatGroups.slice(0, -1).some((node) => hasTrailingComment(node.node)) || (groups[cutoff] && hasLeadingComment(groups[cutoff][0].node));
    if (groups.length <= cutoff && !hasComment && groups[0][0].node.type !== "LogicalExpression") {
      return group$1(oneLine);
    }
    const expanded = concat$2([printGroup(groups[0]), shouldMerge ? concat$2(groups.slice(1, 2).map(printGroup)) : "", printIndentedGroup(groups.slice(shouldMerge ? 2 : 1))]);
    const callExpressionCount = printedNodes.filter((tuple) => tuple.node.type === "CallExpression").length;
    if (hasComment || callExpressionCount >= 3 || printedGroups.slice(0, -1).some(willBreak)) {
      return group$1(expanded);
    }
    return concat$2([willBreak(oneLine) ? breakParent$2 : "", conditionalGroup$1([oneLine, expanded])]);
  }
  ...
  function printJSXElement(path, options, print) {
    ...
    if (n.children.length === 1 && n.children[0].type === "JSXExpressionContainer" && (n.children[0].expression.type === "TemplateLiteral" || n.children[0].expression.type === "TaggedTemplateExpression")) {
      return concat$2([openingLines, concat$2(path.map(print, "children")), closingLines]);
    }
    ...
    for (let i = children.length - 2; i >= 0; i--) {
      ...
      const isPairOfHardlines = children[i] === hardline$2 && children[i + 1] === "" && children[i + 2] === hardline$2;
      const isLineFollowedByJSXWhitespace = (children[i] === softline$1 || children[i] === hardline$2) && children[i + 1] === "" && children[i + 2] === jsxWhitespace;
      const isJSXWhitespaceFollowedByLine = children[i] === jsxWhitespace && children[i + 1] === "" && (children[i + 2] === softline$1 || children[i + 2] === hardline$2);
    ...
    while (children.length && (isLineNext(util$5.getLast(children)) || isEmpty(util$5.getLast(children)))) {
      children.pop();
    }
    ...
    const content = containsText ? fill$1(multilineChildren) : group$1(concat$2(multilineChildren), {shouldBreak: true});
    const multiLineElem = group$1(concat$2([openingLines, indent$2(concat$2([hardline$2, content])), hardline$2, closingLines]));
    if (forcedBreak) {
  ...
  function isMemberish(node) {
    return (node.type === "MemberExpression" || (node.type === "BindExpression" && node.object));
  }
  ...
  function printBinaryishExpressions(path, print, options, isNested, isInsideParenthesis) {
    ...
    if (isBinaryish(node)) {
      if (util$5.shouldFlatten(node.operator, node.left.operator)) {
        parts = parts.concat(path.call((left) => printBinaryishExpressions(left, print, options, true, isInsideParenthesis), "left"));
      } else {
        parts.push(path.call(print, "left"));
      }
      const right = concat$2([node.operator, shouldInlineLogicalExpression(node) ? " " : line$1, path.call(print, "right")]);
      const parent = path.getParentNode();
  ...
  function printAssignment(leftNode, printedLeft, operator, rightNode, printedRight, options) {
    ...
    const canBreak = (isBinaryish(rightNode) && !shouldInlineLogicalExpression(rightNode)) || (rightNode.type === "ConditionalExpression" && isBinaryish(rightNode.test) && !shouldInlineLogicalExpression(rightNode.test)) || ((leftNode.type === "Identifier" || isStringLiteral(leftNode) || leftNode.type === "MemberExpression") && (isStringLiteral(rightNode) || isMemberExpressionChain(rightNode)));
  ...
  function hasLeadingOwnLineComment(text, node) {
    ...
    const res = node.comments && node.comments.some((comment) => comment.leading && util$5.hasNewline(text, util$5.locEnd(comment)));
    return res;
  }
  function hasNakedLeftSide(node) {
    return (node.type === "AssignmentExpression" || node.type === "BinaryExpression" || node.type === "LogicalExpression" || node.type === "ConditionalExpression" || node.type === "CallExpression" || node.type === "MemberExpression" || node.type === "SequenceExpression" || node.type === "TaggedTemplateExpression" || (node.type === "BindExpression" && !node.object) || (node.type === "UpdateExpression" && !node.prefix));
  }
  ...
  function isNodeStartingWithDeclare(node, options) {
    ...
    return (options.originalText.slice(0, util$5.locStart(node)).match(/declare[ \t]*$/) || options.originalText.slice(node.range[0], node.range[1]).startsWith("declare "));
  }
  function shouldHugType(node) {
    ...
    if (node.type === "UnionTypeAnnotation" || node.type === "TSUnionType") {
      ...
      const objectCount = node.types.filter((n) => n.type === "ObjectTypeAnnotation" || n.type === "TSTypeLiteral" || n.type === "GenericTypeAnnotation" || n.type === "TSTypeReference").length;
  ...
  function shouldHugArguments(fun) {
    return (fun && fun.params && fun.params.length === 1 && !fun.params[0].comments && (fun.params[0].type === "ObjectPattern" || (fun.params[0].type === "Identifier" && fun.params[0].typeAnnotation && fun.params[0].typeAnnotation.type === "TypeAnnotation" && isObjectType(fun.params[0].typeAnnotation.typeAnnotation)) || (fun.params[0].type === "FunctionTypeParam" && isObjectType(fun.params[0].typeAnnotation))) && !fun.rest);
  }
  function templateLiteralHasNewLines(template) {
    return template.quasis.some((quasi) => quasi.value.raw.includes("\n"));
  }
  function isTemplateOnItsOwnLine(n, text) {
    return (((n.type === "TemplateLiteral" && templateLiteralHasNewLines(n)) || (n.type === "TaggedTemplateExpression" && templateLiteralHasNewLines(n.quasi))) && !util$5.hasNewline(text, util$5.locStart(n), {backwards: true}));
  }
  function printArrayItems(path, options, printPath, print) {
    ...
    path.each((childPath) => {
      ...
      if (childPath.getValue() && util$5.isNextLineEmpty(options.originalText, childPath.getValue())) {
  ...
  function printAstToDoc$1(ast, options, addAlignmentSize) {
    ...
    function printGenerically(path, args) {
      ...
      const parent = path.getParentNode(0);
      let res;
      if (((node && node.type === "JSXElement") || (parent && (parent.type === "UnionTypeAnnotation" || parent.type === "TSUnionType" || ((parent.type === "ClassDeclaration" || parent.type === "ClassExpression") && parent.superClass === node)))) && !hasIgnoreComment(path)) {
          ...
          case "fill":
            {
              ...
              const firstAndSecondContentFlatCmd = [ind, MODE_FLAT, concat$7([content, whitespace, secondContent])];
            ...
            switch (mode) {
              ...
              case MODE_BREAK:
                ...
                if (doc.literal) {
                  ...
                  const indentString = options.useTabs ? "\t".repeat(ind.indent + ind.align.tabs) : " ".repeat(length);
  ...
  function printChildren$1(flatChildren, print, indent, colors, opts) {
    ...
    }).filter((value) => value.trim().length).join(opts.edgeSpacing);
  }
  function printAttributes$1(attributes, indent, colors, opts) {
    return attributes.sort().map((attribute) => {
  ...
  const print$3 = (element, print, indent, opts, colors) => {
    if (element.nodeType === 3) {
      return element.data.split('\n').map((text) => text.trimLeft()).filter((text) => text.length).join(' ');
  ...
  const printImmutable$1 = (val, print, indent, opts, colors, immutableDataStructureName, isMap) => {
    ...
    const openTag = _ref2[0],
        closeTag = _ref2[1];
    let result = IMMUTABLE_NAMESPACE + immutableDataStructureName + SPACE$1 + openTag + opts.edgeSpacing;
  ...
  const print$10 = (element, print, indent, opts, colors) => {
    ...
    const hasProps = !!Object.keys(element.props).filter((propName) => propName !== 'children').length;
  ...
  function normalizeOptions$1(opts) {
    const result = {};
    Object.keys(DEFAULTS).forEach((key) => result[key] = opts.hasOwnProperty(key) ? key === 'theme' ? normalizeTheme(opts.theme) : opts[key] : DEFAULTS[key]);
    if (result.min) {
      result.indent = 0;
    }
    return result;
  ...
  const format$2 = (value) => typeof value === 'function' ? value.toString() : index$38(value, {min: true});
  ...
  var deprecated = {deprecationWarning: deprecationWarning$1};
  ...
  const deprecated$2 = {useFlowParser: (config) => `  The ${'"useFlowParser"'} option is deprecated. Use ${'"parser"'} instead.
  ...
  var docDebug = {printDocToDebug: function(doc) {
  ...
  var createExplorer$1 = function(options) {
    ...
    function load(searchPath, configPath) {
      ...
      return isDirectory(absoluteSearchPath).then(function(searchPathIsDirectory) {
        var directory = (searchPathIsDirectory) ? absoluteSearchPath : path$2.dirname(absoluteSearchPath);
    ...
    function searchDirectory(directory) {
      ...
      var result = Promise.resolve().then(function() {
        ...
        var nextDirectory = (splitPath.length > 1) ? splitPath.slice(0, -1).join(path$2.sep) : null;
  ...
  function findSiblingAncestors(startNodeAndParents, endNodeAndParents) {
    ...
    for (const endParent of endNodeAndParents.parentNodes) {
      if (endParent.type !== "Program" && endParent.type !== "File" && util$1.locStart(endParent) >= util$1.locStart(startNodeAndParents.node)) {
    ...
    for (const startParent of startNodeAndParents.parentNodes) {
      if (startParent.type !== "Program" && startParent.type !== "File" && util$1.locEnd(startParent) <= util$1.locEnd(endNodeAndParents.node)) {
