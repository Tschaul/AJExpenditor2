...
(function(process) {
  ...
  var _require2 = require('../../jest-message-util/build/index'),
      formatStackTrace = _require2.formatStackTrace,
      separateMessageFromStack = _require2.separateMessageFromStack;
  var _require3 = require('../../jest-matcher-utils/build/index'),
      RECEIVED_BG = _require3.RECEIVED_BG,
  ...
  var createMatcher = function createMatcher(matcherName) {
    return function(actual, expected) {
      ...
      if (typeof actual !== 'function') {
        throw new Error(matcherHint(matcherName, 'function', getType(value)) + '\n\n' + 'Received value must be a function, but instead ' + ('"' + getType(actual) + '" was found'));
      }
      try {
        actual();
      } catch (e) {
  ...
  var toThrowMatchingStringOrRegexp = function toThrowMatchingStringOrRegexp(name, error, pattern, value) {
    ...
    var message = pass ? function() {
      return (matcherHint('.not' + name, 'function', getType(value)) + '\n\n' + 'Expected the function not to throw an error matching:\n' + ('  ' + printExpected(value) + '\n') + printActualErrorMessage(error));
    } : function() {
      return (matcherHint(name, 'function', getType(value)) + '\n\n' + 'Expected the function to throw an error matching:\n' + ('  ' + printExpected(value) + '\n') + printActualErrorMessage(error));
    };
    return {
      message: message,
      pass: pass
  ...
  var toThrowMatchingErrorInstance = function toThrowMatchingErrorInstance(name, error, expectedError) {
    ...
    var message = pass ? function() {
      return (matcherHint('.not' + name, 'function', 'error') + '\n\n' + 'Expected the function not to throw an error matching:\n' + ('  ' + printExpected(expectedError) + '\n') + printActualErrorMessage(error));
    } : function() {
      return (matcherHint(name, 'function', 'error') + '\n\n' + 'Expected the function to throw an error matching:\n' + ('  ' + printExpected(expectedError) + '\n') + printActualErrorMessage(error));
    };
    return {
      message: message,
      pass: pass
  ...
  var printActualErrorMessage = function printActualErrorMessage(error) {
    if (error) {
      ...
      return ('Instead, it threw:\n' + RECEIVED_COLOR('  ' + highlightTrailingWhitespace(message, RECEIVED_BG) + formatStackTrace(stack, {
        rootDir: process.cwd(),
        testMatch: []
      }, {noStackTrace: false})));
    }
