...
(function(process) {
  ...
  (function(ts) {
    ...
    function arrayToSet(array, makeKey) {
      return arrayToMap(array, makeKey || (function(s) {
        return s;
      }), function() {
        return true;
      });
    ...
    var filesMatcher = {
      ...
      replaceWildcardCharacter: function(match) {
    ...
    function getSubPatternFromSpec(spec, basePath, usage, _a) {
      var singleAsteriskRegexFragment = _a.singleAsteriskRegexFragment,
          doubleAsteriskRegexFragment = _a.doubleAsteriskRegexFragment,
          replaceWildcardCharacter = _a.replaceWildcardCharacter;
      var subpattern = "";
    ...
    function getSupportedExtensions(options, extraFileExtensions) {
      ...
      return deduplicate(allSupportedExtensions.concat(extraFileExtensions.map(function(e) {
        return e.extension;
      })));
    }
    ...
    function tryGetExtensionFromPath(path) {
      return find(ts.supportedTypescriptExtensionsForExtractExtension, function(e) {
        return fileExtensionIs(path, e);
      }) || find(ts.supportedJavascriptExtensions, function(e) {
        return fileExtensionIs(path, e);
      });
    }
        ...
        var nodeSystem = {
          ...
          watchFile: function(fileName, callback, pollingInterval) {
            ...
            function fileChanged(curr, prev) {
              ...
              var eventKind = created ? FileWatcherEventKind.Created : deleted ? FileWatcherEventKind.Deleted : FileWatcherEventKind.Changed;
          ...
          debugMode: ts.some(process.execArgv, function(arg) {
  ...
  (function(ts) {
    ...
    function getDeclarationOfKind(symbol, kind) {
      ...
      if (declarations) {
        for (var _i = 0,
            declarations_1 = declarations; _i < declarations_1.length; _i++) {
    ...
    function moduleResolutionIsEqualTo(oldResolution, newResolution) {
      return oldResolution.isExternalLibraryImport === newResolution.isExternalLibraryImport && oldResolution.extension === newResolution.extension && oldResolution.resolvedFileName === newResolution.resolvedFileName && packageIdIsEqual(oldResolution.packageId, newResolution.packageId);
    }
    ...
    function aggregateChildData(node) {
      if (!(node.flags & 262144)) {
        var thisNodeOrAnySubNodesHasError = ((node.flags & 32768) !== 0) || ts.forEachChild(node, containsParseError);
    ...
    function isRecognizedTripleSlashComment(text, commentPos, commentEnd) {
      if (text.charCodeAt(commentPos + 1) === 47 && commentPos + 2 < commentEnd && text.charCodeAt(commentPos + 2) === 47) {
        ...
        return textSubStr.match(ts.fullTripleSlashReferencePathRegEx) || textSubStr.match(ts.fullTripleSlashAMDReferencePathRegEx) || textSubStr.match(fullTripleSlashReferenceTypeReferenceDirectiveRegEx) || textSubStr.match(defaultLibReferenceRegEx) ? true : false;
      }
      return false;
    }
    ts.isRecognizedTripleSlashComment = isRecognizedTripleSlashComment;
    function isPinnedComment(text, comment) {
      return text.charCodeAt(comment.pos + 1) === 42 && text.charCodeAt(comment.pos + 2) === 33;
    }
    ...
    function isBlockOrCatchScoped(declaration) {
      return (ts.getCombinedNodeFlags(declaration) & 3) !== 0 || isCatchClauseVariableDeclarationOrBindingElement(declaration);
    }
    ...
    function isAmbientModule(node) {
      return node && node.kind === 233 && (node.name.kind === 9 || isGlobalScopeAugmentation(node));
    }
    ...
    function isBlockScopedContainerTopLevel(node) {
      return node.kind === 265 || node.kind === 233 || ts.isFunctionLike(node);
    }
    ...
    function isConst(node) {
      return !!(ts.getCombinedNodeFlags(node) & 2) || !!(ts.getCombinedModifierFlags(node) & 2048);
    }
    ts.isConst = isConst;
    function isLet(node) {
    ...
    function isPrologueDirective(node) {
      return node.kind === 210 && node.expression.kind === 9;
    }
    ...
    function getJSDocCommentRanges(node, text) {
      var commentRanges = (node.kind === 146 || node.kind === 145 || node.kind === 186 || node.kind === 187 || node.kind === 185) ? ts.concatenate(ts.getTrailingCommentRanges(text, node.pos), ts.getLeadingCommentRanges(text, node.pos)) : ts.getLeadingCommentRanges(text, node.pos);
      return ts.filter(commentRanges, function(comment) {
        return text.charCodeAt(comment.pos + 1) === 42 && text.charCodeAt(comment.pos + 2) === 42 && text.charCodeAt(comment.pos + 3) !== 47;
      });
    }
    ...
    function isObjectLiteralOrClassExpressionMethod(node) {
      return node.kind === 151 && (node.parent.kind === 178 || node.parent.kind === 199);
    }
    ...
    function isSuperProperty(node) {
      var kind = node.kind;
      return (kind === 179 || kind === 180) && node.expression.kind === 97;
    }
    ...
    function nodeCanBeDecorated(node) {
      switch (node.kind) {
        ...
        case 154:
        case 151:
          return node.body !== undefined && node.parent.kind === 229;
        case 146:
          return node.parent.body !== undefined && (node.parent.kind === 152 || node.parent.kind === 151 || node.parent.kind === 154) && node.parent.parent.kind === 229;
      }
      return false;
    }
    ...
    function isJSXTagName(node) {
      var parent = node.parent;
      if (parent.kind === 251 || parent.kind === 250 || parent.kind === 252) {
        ...
        case 99:
          ...
          switch (parent.kind) {
            ...
            case 214:
              ...
              return (forStatement.initializer === node && forStatement.initializer.kind !== 227) || forStatement.condition === node || forStatement.incrementor === node;
            case 215:
            case 216:
              var forInStatement = parent;
              return (forInStatement.initializer === node && forInStatement.initializer.kind !== 227) || forInStatement.expression === node;
            case 184:
            case 202:
    ...
    function isDefaultImport(node) {
      return node.kind === 238 && node.importClause && !!node.importClause.name;
    }
    ...
    function isJSDocConstructSignature(node) {
      return node.kind === 273 && node.parameters.length > 0 && node.parameters[0].name && node.parameters[0].name.escapedText === "new";
    }
    ...
    function getFirstJSDocTag(node, kind) {
      ...
      return ts.find(tags, function(doc) {
    ...
    function getJSDocTags(node) {
      ...
      if (tags === undefined) {
        node.jsDocCache = tags = ts.flatMap(getJSDocCommentsAndTags(node), function(j) {
          return ts.isJSDoc(j) ? j.tags : j;
        });
      }
      return tags;
    }
    ...
    function getJSDocCommentsAndTags(node) {
      ...
      function getJSDocCommentsAndTagsWorker(node) {
        var parent = node.parent;
        var isInitializerOfVariableDeclarationInStatement = isVariableLike(parent) && parent.initializer === node && parent.parent.parent.kind === 208;
        var isVariableOfVariableDeclarationStatement = isVariableLike(node) && parent.parent.kind === 208;
        var variableStatementNode = isInitializerOfVariableDeclarationInStatement ? parent.parent.parent : isVariableOfVariableDeclarationStatement ? parent.parent : undefined;
        if (variableStatementNode) {
          getJSDocCommentsAndTagsWorker(variableStatementNode);
        }
        var isSourceOfAssignmentExpressionStatement = parent && parent.parent && parent.kind === 194 && parent.operatorToken.kind === 58 && parent.parent.kind === 210;
        if (isSourceOfAssignmentExpressionStatement) {
          getJSDocCommentsAndTagsWorker(parent.parent);
        }
        var isModuleDeclaration = node.kind === 233 && parent && parent.kind === 233;
    ...
    function getJSDocParameterTags(param) {
      if (param.name && ts.isIdentifier(param.name)) {
        ...
        return getJSDocTags(param.parent).filter(function(tag) {
          return ts.isJSDocParameterTag(tag) && ts.isIdentifier(tag.name) && tag.name.escapedText === name_1;
        });
      }
      return undefined;
    }
    ...
    function getTypeParameterFromJsDoc(node) {
      ...
      return ts.find(typeParameters, function(p) {
    ...
    function getJSDocType(node) {
      ...
      if (!tag && node.kind === 146) {
        ...
        if (paramTags) {
          tag = ts.find(paramTags, function(tag) {
    ...
    function isRestParameter(node) {
      if (isInJavaScriptFile(node)) {
        if (node.type && node.type.kind === 274 || ts.forEach(getJSDocParameterTags(node), function(t) {
          return t.typeExpression && t.typeExpression.type.kind === 274;
        })) {
          return true;
        }
      }
      ...
      while (true) {
        switch (parent.kind) {
          case 194:
            ...
            return isAssignmentOperator(binaryOperator) && parent.left === node ? binaryOperator === 58 ? 1 : 2 : 0;
          case 192:
          case 193:
    ...
    function isLiteralComputedPropertyDeclarationName(node) {
      return (node.kind === 9 || node.kind === 8) && node.parent.kind === 144 && ts.isDeclaration(node.parent.parent);
    }
    ...
    function isAliasSymbolDeclaration(node) {
      return node.kind === 237 || node.kind === 236 || node.kind === 239 && !!node.name || node.kind === 240 || node.kind === 242 || node.kind === 246 || node.kind === 243 && exportAssignmentIsAlias(node);
    }
    ...
    function getHeritageClause(clauses, kind) {
      if (clauses) {
        for (var _i = 0,
            clauses_1 = clauses; _i < clauses_1.length; _i++) {
    ...
    function isAsyncFunction(node) {
      switch (node.kind) {
        ...
        case 187:
        case 151:
          return node.body !== undefined && node.asteriskToken === undefined && hasModifier(node, 256);
      }
      return false;
    }
    ...
    function isDynamicName(name) {
      return name.kind === 144 && !isStringOrNumericLiteral(name.expression) && !isWellKnownSymbolSyntactically(name.expression);
    }
    ...
    function nodeStartsNewLexicalEnvironment(node) {
      var kind = node.kind;
      return kind === 152 || kind === 186 || kind === 228 || kind === 187 || kind === 151 || kind === 153 || kind === 154 || kind === 233 || kind === 265;
    }
    ts.nodeStartsNewLexicalEnvironment = nodeStartsNewLexicalEnvironment;
    function nodeIsSynthesized(range) {
      return ts.positionIsSynthesized(range.pos) || ts.positionIsSynthesized(range.end);
    }
    ...
    function createDiagnosticCollection() {
      ...
      function reattachFileDiagnostics(newFile) {
        ts.forEach(fileDiagnostics.get(newFile.fileName), function(diagnostic) {
          return diagnostic.file = newFile;
        });
      }
    ...
    function escapeString(s, quoteChar) {
      var escapedCharsRegExp = quoteChar === 96 ? backtickQuoteEscapedCharsRegExp : quoteChar === 39 ? singleQuoteEscapedCharsRegExp : doubleQuoteEscapedCharsRegExp;
    ...
    function getDeclarationEmitOutputFilePath(sourceFile, host) {
      ...
      var path = outputDir ? getSourceFilePathInNewDir(sourceFile, host, outputDir) : sourceFile.fileName;
    ...
    function getSourceFilesToEmit(host, targetSourceFile) {
      var options = host.getCompilerOptions();
      var isSourceFileFromExternalLibrary = function(file) {
        return host.isSourceFileFromExternalLibrary(file);
      };
      if (options.outFile || options.out) {
        ...
        return ts.filter(sourceFiles, function(sourceFile) {
          return sourceFileMayBeEmitted(sourceFile, options, isSourceFileFromExternalLibrary);
        });
      }
    }
    ...
    function getAllAccessorDeclarations(declarations, accessor) {
      ...
      if (hasDynamicName(accessor)) {
        ...
        ts.forEach(declarations, function(member) {
          if ((member.kind === 153 || member.kind === 154) && hasModifier(member, 32) === hasModifier(accessor, 32)) {
    ...
    function emitDetachedComments(text, lineMap, writer, writeComment, node, newLine, removeComments) {
      ...
      if (leadingComments) {
        ...
        for (var _i = 0,
            leadingComments_1 = leadingComments; _i < leadingComments_1.length; _i++) {
    ...
    function writeCommentRange(text, lineMap, writer, commentPos, commentEnd, newLine) {
      if (text.charCodeAt(commentPos + 1) === 42) {
        ...
        for (var pos = commentPos,
            currentLine = firstCommentLineAndCharacter.line; pos < commentEnd; currentLine++) {
    ...
    function isLogicalOperator(token) {
      return token === 54 || token === 53 || token === 51;
    }
    ...
    function tryGetClassExtendingExpressionWithTypeArguments(node) {
      if (node.kind === 201 && node.parent.token === 85 && ts.isClassLike(node.parent.parent)) {
        return node.parent.parent;
      }
    }
    ts.tryGetClassExtendingExpressionWithTypeArguments = tryGetClassExtendingExpressionWithTypeArguments;
    function isAssignmentExpression(node, excludeCompoundAssignment) {
      return ts.isBinaryExpression(node) && (excludeCompoundAssignment ? node.operatorToken.kind === 58 : isAssignmentOperator(node.operatorToken.kind)) && ts.isLeftHandSideExpression(node.left);
    }
    ts.isAssignmentExpression = isAssignmentExpression;
    function isDestructuringAssignment(node) {
      if (isAssignmentExpression(node, true)) {
        var kind = node.left.kind;
        return kind === 178 || kind === 177;
      }
      return false;
    }
    ...
    function isExpressionWithTypeArgumentsInClassImplementsClause(node) {
      return node.kind === 201 && isEntityNameExpression(node.expression) && node.parent && node.parent.token === 108 && node.parent.parent && ts.isClassLike(node.parent.parent);
    }
    ts.isExpressionWithTypeArgumentsInClassImplementsClause = isExpressionWithTypeArgumentsInClassImplementsClause;
    function isEntityNameExpression(node) {
      return node.kind === 71 || node.kind === 179 && isEntityNameExpression(node.expression);
    }
    ts.isEntityNameExpression = isEntityNameExpression;
    function isRightSideOfQualifiedNameOrPropertyAccess(node) {
      return (node.parent.kind === 143 && node.parent.right === node) || (node.parent.kind === 179 && node.parent.name === node);
    }
    ts.isRightSideOfQualifiedNameOrPropertyAccess = isRightSideOfQualifiedNameOrPropertyAccess;
    function isEmptyObjectLiteral(expression) {
      return expression.kind === 178 && expression.properties.length === 0;
    }
    ts.isEmptyObjectLiteral = isEmptyObjectLiteral;
    function isEmptyArrayLiteral(expression) {
      return expression.kind === 177 && expression.elements.length === 0;
    }
    ...
    function tryExtractTypeScriptExtension(fileName) {
      return ts.find(ts.supportedTypescriptExtensionsForExtractExtension, function(extension) {
    ...
    function getEnumMembers(enumObject) {
      ...
      return ts.stableSort(result, function(x, y) {
    ...
    function getDeclarationModifierFlagsFromSymbol(s) {
      ...
      if (getCheckFlags(s) & 6) {
        ...
        var accessModifier = checkFlags & 256 ? 8 : checkFlags & 64 ? 4 : 16;
  ...
  (function(ts) {
    ...
    function isEntityName(node) {
      var kind = node.kind;
      return kind === 143 || kind === 71;
    }
    ts.isEntityName = isEntityName;
    function isPropertyName(node) {
      var kind = node.kind;
      return kind === 71 || kind === 9 || kind === 8 || kind === 144;
    }
    ts.isPropertyName = isPropertyName;
    function isBindingName(node) {
      var kind = node.kind;
      return kind === 71 || kind === 174 || kind === 175;
    }
    ...
    function isClassElement(node) {
      var kind = node.kind;
      return kind === 152 || kind === 149 || kind === 151 || kind === 153 || kind === 154 || kind === 157 || kind === 206 || kind === 247;
    }
    ...
    function isTypeElement(node) {
      var kind = node.kind;
      return kind === 156 || kind === 155 || kind === 148 || kind === 150 || kind === 157 || kind === 247;
    }
    ts.isTypeElement = isTypeElement;
    function isObjectLiteralElementLike(node) {
      var kind = node.kind;
      return kind === 261 || kind === 262 || kind === 263 || kind === 151 || kind === 153 || kind === 154 || kind === 247;
    }
    ts.isObjectLiteralElementLike = isObjectLiteralElementLike;
    function isTypeNodeKind(kind) {
      return (kind >= 158 && kind <= 173) || kind === 119 || kind === 133 || kind === 134 || kind === 122 || kind === 136 || kind === 137 || kind === 99 || kind === 105 || kind === 139 || kind === 95 || kind === 130 || kind === 201;
    }
    ...
    function isTemplateLiteral(node) {
      var kind = node.kind;
      return kind === 196 || kind === 13;
    }
    ts.isTemplateLiteral = isTemplateLiteral;
    function isLeftHandSideExpressionKind(kind) {
      return kind === 179 || kind === 180 || kind === 182 || kind === 181 || kind === 249 || kind === 250 || kind === 183 || kind === 177 || kind === 185 || kind === 178 || kind === 199 || kind === 186 || kind === 71 || kind === 12 || kind === 8 || kind === 9 || kind === 13 || kind === 196 || kind === 86 || kind === 95 || kind === 99 || kind === 101 || kind === 97 || kind === 91 || kind === 203 || kind === 204;
    }
    ...
    function isUnaryExpressionKind(kind) {
      return kind === 192 || kind === 193 || kind === 188 || kind === 189 || kind === 190 || kind === 191 || kind === 184 || isLeftHandSideExpressionKind(kind);
    }
    ...
    function isUnaryExpressionWithWrite(expr) {
      switch (expr.kind) {
        case 193:
          return true;
        case 192:
          return expr.operator === 43 || expr.operator === 44;
        default:
          return false;
      }
    }
    ...
    function isExpressionKind(kind) {
      return kind === 195 || kind === 197 || kind === 187 || kind === 194 || kind === 198 || kind === 202 || kind === 200 || kind === 289 || isUnaryExpressionKind(kind);
    }
    ...
    function isConciseBody(node) {
      return ts.isBlock(node) || isExpression(node);
    }
    ...
    function isForInitializer(node) {
      return ts.isVariableDeclarationList(node) || isExpression(node);
    }
    ts.isForInitializer = isForInitializer;
    function isModuleBody(node) {
      var kind = node.kind;
      return kind === 234 || kind === 233 || kind === 71;
    }
    ts.isModuleBody = isModuleBody;
    function isNamespaceBody(node) {
      var kind = node.kind;
      return kind === 234 || kind === 233;
    }
    ts.isNamespaceBody = isNamespaceBody;
    function isJSDocNamespaceBody(node) {
      var kind = node.kind;
      return kind === 71 || kind === 233;
    }
    ts.isJSDocNamespaceBody = isJSDocNamespaceBody;
    function isNamedImportBindings(node) {
      var kind = node.kind;
      return kind === 241 || kind === 240;
    }
    ...
    function isDeclarationKind(kind) {
      return kind === 187 || kind === 176 || kind === 229 || kind === 199 || kind === 152 || kind === 232 || kind === 264 || kind === 246 || kind === 228 || kind === 186 || kind === 153 || kind === 239 || kind === 237 || kind === 242 || kind === 230 || kind === 253 || kind === 151 || kind === 150 || kind === 233 || kind === 236 || kind === 240 || kind === 146 || kind === 261 || kind === 149 || kind === 148 || kind === 154 || kind === 262 || kind === 231 || kind === 145 || kind === 226 || kind === 283;
    }
    function isDeclarationStatementKind(kind) {
      return kind === 228 || kind === 247 || kind === 229 || kind === 230 || kind === 231 || kind === 232 || kind === 233 || kind === 238 || kind === 237 || kind === 244 || kind === 243 || kind === 236;
    }
    function isStatementKindButNotDeclarationKind(kind) {
      return kind === 218 || kind === 217 || kind === 225 || kind === 212 || kind === 210 || kind === 209 || kind === 215 || kind === 216 || kind === 214 || kind === 211 || kind === 222 || kind === 219 || kind === 221 || kind === 223 || kind === 224 || kind === 208 || kind === 213 || kind === 220 || kind === 287 || kind === 291 || kind === 290;
    }
    ...
    function isModuleReference(node) {
      var kind = node.kind;
      return kind === 248 || kind === 143 || kind === 71;
    }
    ts.isModuleReference = isModuleReference;
    function isJsxTagNameExpression(node) {
      var kind = node.kind;
      return kind === 99 || kind === 71 || kind === 179;
    }
    ts.isJsxTagNameExpression = isJsxTagNameExpression;
    function isJsxChild(node) {
      var kind = node.kind;
      return kind === 249 || kind === 256 || kind === 250 || kind === 10;
    }
    ts.isJsxChild = isJsxChild;
    function isJsxAttributeLike(node) {
      var kind = node.kind;
      return kind === 253 || kind === 255;
    }
    ts.isJsxAttributeLike = isJsxAttributeLike;
    function isStringLiteralOrJsxExpression(node) {
      var kind = node.kind;
      return kind === 9 || kind === 256;
    }
    ts.isStringLiteralOrJsxExpression = isStringLiteralOrJsxExpression;
    function isJsxOpeningLikeElement(node) {
      var kind = node.kind;
      return kind === 251 || kind === 250;
    }
    ts.isJsxOpeningLikeElement = isJsxOpeningLikeElement;
    function isCaseOrDefaultClause(node) {
      var kind = node.kind;
      return kind === 257 || kind === 258;
    }
  ...
  (function(ts) {
    ...
    function visitNodes(cbNode, cbNodes, nodes) {
      if (nodes) {
        ...
        for (var _i = 0,
            nodes_1 = nodes; _i < nodes_1.length; _i++) {
          var node = nodes_1[_i];
    ...
    function forEachChild(node, cbNode, cbNodes) {
      ...
      switch (node.kind) {
        ...
        case 262:
          return visitNodes(cbNode, cbNodes, node.decorators) || visitNodes(cbNode, cbNodes, node.modifiers) || visitNode(cbNode, node.name) || visitNode(cbNode, node.questionToken) || visitNode(cbNode, node.equalsToken) || visitNode(cbNode, node.objectAssignmentInitializer);
        case 263:
        ...
        case 226:
        case 176:
          return visitNodes(cbNode, cbNodes, node.decorators) || visitNodes(cbNode, cbNodes, node.modifiers) || visitNode(cbNode, node.propertyName) || visitNode(cbNode, node.dotDotDotToken) || visitNode(cbNode, node.name) || visitNode(cbNode, node.questionToken) || visitNode(cbNode, node.type) || visitNode(cbNode, node.initializer);
        case 160:
        case 161:
        case 155:
        case 156:
        ...
        case 228:
        case 187:
          return visitNodes(cbNode, cbNodes, node.decorators) || visitNodes(cbNode, cbNodes, node.modifiers) || visitNode(cbNode, node.asteriskToken) || visitNode(cbNode, node.name) || visitNode(cbNode, node.questionToken) || visitNodes(cbNode, cbNodes, node.typeParameters) || visitNodes(cbNode, cbNodes, node.parameters) || visitNode(cbNode, node.type) || visitNode(cbNode, node.equalsGreaterThanToken) || visitNode(cbNode, node.body);
        case 159:
        ...
        case 181:
        case 182:
          return visitNode(cbNode, node.expression) || visitNodes(cbNode, cbNodes, node.typeArguments) || visitNodes(cbNode, cbNodes, node.arguments);
        case 183:
        ...
        case 250:
        case 251:
          return visitNode(cbNode, node.tagName) || visitNode(cbNode, node.attributes);
        case 254:
          return visitNodes(cbNode, cbNodes, node.properties);
        case 253:
          return visitNode(cbNode, node.name) || visitNode(cbNode, node.initializer);
        ...
        case 284:
          if (node.isNameFirst) {
            return visitNode(cbNode, node.name) || visitNode(cbNode, node.typeExpression);
          } else {
            return visitNode(cbNode, node.typeExpression) || visitNode(cbNode, node.name);
          }
        case 280:
        ...
        case 283:
          if (node.typeExpression && node.typeExpression.kind === 267) {
            return visitNode(cbNode, node.typeExpression) || visitNode(cbNode, node.fullName);
          } else {
            return visitNode(cbNode, node.fullName) || visitNode(cbNode, node.typeExpression);
          }
        case 285:
          for (var _i = 0,
              _a = node.jsDocPropertyTags; _i < _a.length; _i++) {
            var tag = _a[_i];
    ...
    (function(Parser) {
      ...
      function parseJsonText(fileName, sourceText) {
        ...
        } else if (token() === 17 || lookAhead(function() {
          return token() === 9;
        })) {
      ...
      function addJSDocComment(node) {
        ...
        if (comments) {
          for (var _i = 0,
              comments_2 = comments; _i < comments_2.length; _i++) {
      ...
      function createNode(kind, pos) {
        ...
        return ts.isNodeKind(kind) ? new NodeConstructor(kind, pos, pos) : kind === 71 ? new IdentifierConstructor(kind, pos, pos) : new TokenConstructor(kind, pos, pos);
      }
      ...
      function isLiteralPropertyName() {
        return ts.tokenIsIdentifierOrKeyword(token()) || token() === 9 || token() === 8;
      }
      ...
      function canFollowModifier() {
        return token() === 21 || token() === 17 || token() === 39 || token() === 24 || isLiteralPropertyName();
      }
      function nextTokenCanFollowDefaultKeyword() {
        nextToken();
        return token() === 75 || token() === 89 || token() === 109 || (token() === 117 && lookAhead(nextTokenIsClassKeywordOnSameLine)) || (token() === 120 && lookAhead(nextTokenIsFunctionKeywordOnSameLine));
      }
        ...
        if (node) {
          switch (node.kind) {
            ...
            case 151:
              ...
              var nameIsConstructor = methodDeclaration.name.kind === 71 && methodDeclaration.name.originalKeywordKind === 123;
      ...
      function parseJSDocUnknownOrNullableType() {
        ...
        nextToken();
        if (token() === 26 || token() === 18 || token() === 20 || token() === 29 || token() === 58 || token() === 49) {
      ...
      function isStartOfParameter() {
        return token() === 24 || isIdentifierOrPattern() || ts.isModifierKind(token()) || token() === 57 || isStartOfType();
      }
      ...
      function isTypeMemberStart() {
        ...
        if (idToken) {
          return token() === 19 || token() === 27 || token() === 55 || token() === 56 || token() === 26 || canParseSemicolon();
        }
        return false;
      }
      ...
      function isUnambiguouslyStartOfFunctionType() {
        ...
        if (skipParameterStart()) {
          if (token() === 56 || token() === 26 || token() === 55 || token() === 58) {
            return true;
          }
          if (token() === 20) {
            nextToken();
      ...
      function isStartOfExpressionStatement() {
        return token() !== 17 && token() !== 89 && token() !== 75 && token() !== 57 && isStartOfExpression();
      }
      ...
      function parseYieldExpression() {
        ...
        nextToken();
        if (!scanner.hasPrecedingLineBreak() && (token() === 39 || isStartOfExpression())) {
      ...
      function tryParseParenthesizedArrowFunctionExpression() {
        ...
        arrowFunction.body = (lastToken === 36 || lastToken === 17) ? parseArrowFunctionExpressionBody(isAsync) : parseIdentifier();
      ...
      function parseArrowFunctionExpressionBody(isAsync) {
        ...
        if (token() !== 25 && token() !== 89 && token() !== 75 && isStartOfStatement() && !isStartOfExpressionStatement()) {
      ...
      function parseBinaryExpressionRest(precedence, leftOperand) {
        while (true) {
          ...
          var consumeCurrentOperator = token() === 40 ? newPrecedence >= precedence : newPrecedence > precedence;
      ...
      function parseUnaryExpressionOrHigher() {
        if (isUpdateExpression()) {
          ...
          return token() === 40 ? parseBinaryExpressionRest(getBinaryOperatorPrecedence(), updateExpression) : updateExpression;
        }
      ...
      function parseJsxElementName() {
        scanJsxIdentifier();
        var expression = token() === 99 ? parseTokenNode() : parseIdentifierName();
        while (parseOptional(23)) {
      ...
      function parseMemberExpressionRest(expression) {
        while (true) {
          ...
          if (token() === 13 || token() === 14) {
            ...
            tagExpression.template = token() === 13 ? parseLiteralNode() : parseTemplateExpression();
      ...
      function parseArgumentOrArrayLiteralElement() {
        return token() === 24 ? parseSpreadElement() : token() === 26 ? createNode(200) : parseAssignmentExpressionOrHigher();
      }
      ...
      function isDeclaration() {
        while (true) {
          switch (token()) {
            ...
            case 91:
              nextToken();
              return token() === 9 || token() === 39 || token() === 17 || ts.tokenIsIdentifierOrKeyword(token());
            case 84:
              nextToken();
              if (token() === 58 || token() === 39 || token() === 17 || token() === 79 || token() === 118) {
                return true;
              }
              continue;
            case 115:
      ...
      function parsePropertyDeclaration(fullStart, decorators, modifiers, name, questionToken) {
        ...
        property.questionToken = questionToken;
        property.type = parseTypeAnnotation();
        property.initializer = ts.hasModifier(property, 32) ? allowInAnd(parseNonParameterInitializer) : doOutsideOfContext(4096 | 2048, parseNonParameterInitializer);
        parseSemicolon();
        return addJSDocComment(finishNode(property));
      }
      ...
      function parseClassElement() {
        ...
        if (ts.tokenIsIdentifierOrKeyword(token()) || token() === 9 || token() === 8 || token() === 39 || token() === 21) {
      ...
      function parseModuleOrNamespaceDeclaration(fullStart, decorators, modifiers, flags) {
        ...
        node.flags |= flags;
        node.name = parseIdentifier();
        node.body = parseOptional(23) ? parseModuleOrNamespaceDeclaration(getNodePos(), undefined, undefined, 4 | namespaceFlag) : parseModuleBlock();
        return addJSDocComment(finishNode(node));
      }
      ...
      function isExternalModuleReference() {
        return token() === 132 && lookAhead(nextTokenIsOpenParen);
      }
      ...
      function parseImportDeclarationOrImportEqualsDeclaration(fullStart, decorators, modifiers) {
        ...
        if (identifier || token() === 39 || token() === 17) {
      ...
      function parseImportClause(identifier, fullStart) {
        ...
        if (!importClause.name || parseOptional(26)) {
      ...
      function setExternalModuleIndicator(sourceFile) {
        sourceFile.externalModuleIndicator = ts.forEach(sourceFile.statements, function(node) {
          return ts.hasModifier(node, 1) || node.kind === 237 && node.moduleReference.kind === 248 || node.kind === 238 || node.kind === 243 || node.kind === 244 ? node : undefined;
        });
      }
      var JSDocParser;
      (function(JSDocParser) {
        ...
        function parseJSDocCommentWorker(start, length) {
          ...
          function isJsDocStart(content, start) {
            return content.charCodeAt(start) === 47 && content.charCodeAt(start + 1) === 42 && content.charCodeAt(start + 2) === 42 && content.charCodeAt(start + 3) !== 42;
          }
          ...
          function parseParameterOrPropertyTag(atToken, tagName, target) {
            ...
            skipWhitespace();
            var _a = parseBracketNameInPropertyAndParamTag(),
                name = _a.name,
                isBracketed = _a.isBracketed;
            skipWhitespace();
            if (isNameFirst) {
              typeExpression = tryParseTypeExpression();
            }
            var result = target === 1 ? createNode(279, atToken.pos) : createNode(284, atToken.pos);
          ...
          function parseNestedTypeLiteral(typeExpression, name) {
            if (typeExpression && isObjectOrObjectArrayTypeReference(typeExpression.type)) {
              ...
              var children = void 0;
              while (child = tryParse(function() {
                return parseChildParameterOrPropertyTag(1, name);
              })) {
                if (!children) {
                  children = [];
                }
              ...
              switch (token()) {
                case 57:
                  if (canParseTag) {
                    ...
                    if (child && child.kind === 279 && (ts.isIdentifier(child.name) || !escapedTextsEqual(name, child.name.left))) {
                      return false;
                    }
                    return child;
                  }
    ...
    (function(IncrementalParser) {
      ...
      function updateTokenPositionsAndMarkElements(sourceFile, changeStart, changeRangeOldEnd, changeRangeNewEnd, delta, oldText, newText, aggressiveChecks) {
        ...
        function visitArray(array) {
          ...
          if (fullEnd >= changeStart) {
            ...
            for (var _i = 0,
                array_9 = array; _i < array_9.length; _i++) {
              var node = array_9[_i];
              visitNode(node);
            }
            return;
      ...
      function createSyntaxCursor(sourceFile) {
        ...
        return {currentNode: function(position) {
  ...
  (function(ts) {
    ...
    function readConfigFile(fileName, readFile) {
      ...
      return typeof textOrDiagnostic === "string" ? parseConfigFileTextToJson(fileName, textOrDiagnostic) : {
        config: {},
    ...
    function commandLineOptionsToMap(options) {
      return ts.arrayToMap(options, function(option) {
        return option.name;
      });
    }
    ...
    function convertToObjectWorker(sourceFile, errors, knownRootOptions, jsonConversionNotifier) {
      ...
      function convertObjectLiteralExpressionToJson(node, knownOptions, extraKeyDiagnosticMessage, parentOption) {
        ...
        for (var _i = 0,
          ...
          if (typeof keyText !== "undefined" && typeof value !== "undefined") {
            ...
            if (jsonConversionNotifier && (parentOption || knownOptions === knownRootOptions)) {
      ...
      function convertArrayLiteralExpressionToJson(elements, elementOption) {
        return elements.map(function(element) {
          return convertPropertyValueToJson(element, elementOption);
        });
      }
      function convertPropertyValueToJson(valueExpression, option) {
        switch (valueExpression.kind) {
          ...
          case 9:
            ...
            if (option && typeof option.type !== "string") {
              ...
              if (!customOption.type.has(text.toLowerCase())) {
                errors.push(createDiagnosticForInvalidCustomType(customOption, function(message, arg0, arg1) {
                  return ts.createDiagnosticForNodeInSourceFile(sourceFile, valueExpression, message, arg0, arg1);
                }));
              }
            }
            return text;
          ...
          case 178:
            ...
            if (option) {
              var _a = option,
                  elementOptions = _a.elementOptions,
                  extraKeyDiagnosticMessage = _a.extraKeyDiagnosticMessage,
                  optionName = _a.name;
    ...
    function getCompilerOptionValueTypeString(option) {
      return option.type === "list" ? "Array" : typeof option.type === "string" ? option.type : "string";
    }
    ...
    function generateTSConfig(options, fileNames, newLine) {
            ...
            if (optionDefinition) {
              ...
              if (!customTypeMap_1) {
                ...
                if (optionDefinition.type === "list") {
                  result.set(name, value.map(function(element) {
                    return getNameOfCompilerOptionValue(element, customTypeMap_1);
                  }));
                } else {
      ...
      function writeConfigurations() {
        ...
        for (var _i = 0,
            optionDeclarations_1 = ts.optionDeclarations; _i < optionDeclarations_1.length; _i++) {
    ...
    function setConfigFileInOptions(options, configFile) {
      if (configFile) {
        Object.defineProperty(options, "configFile", {
          enumerable: false,
          writable: false,
          value: configFile
        });
    ...
    function parseConfig(json, sourceFile, host, basePath, configFileName, resolutionStack, errors) {
      ...
      var ownConfig = json ? parseOwnConfigOfJson(json, host, basePath, getCanonicalFileName, configFileName, errors) : parseOwnConfigOfJsonSourceFile(sourceFile, host, basePath, getCanonicalFileName, configFileName, errors);
    ...
    function parseOwnConfigOfJsonSourceFile(sourceFile, host, basePath, getCanonicalFileName, configFileName, errors) {
      ...
      var optionsIterator = {
        onSetValidOptionKeyValueInParent: function(parentOption, option, value) {
          ...
          var currentOption = parentOption === "compilerOptions" ? options : parentOption === "typeAcquisition" ? (typeAcquisition || (typeAcquisition = getDefaultTypeAcquisition(configFileName))) : (typingOptionstypeAcquisition || (typingOptionstypeAcquisition = getDefaultTypeAcquisition(configFileName)));
    ...
    function getExtendedConfig(sourceFile, extendedConfigPath, host, basePath, getCanonicalFileName, resolutionStack, errors) {
      var extendedResult = readJsonConfigFile(extendedConfigPath, function(path) {
      ...
      if (isSuccessfulParsedTsconfig(extendedConfig)) {
        ...
        var updatePath_1 = function(path) {
          return ts.isRootedDiskPath(path) ? path : ts.combinePaths(relativeDifference_1, path);
        };
    ...
    function normalizeOptionValue(option, basePath, value) {
      if (option.type === "list") {
        ...
        if (listOption_1.element.isFilePath || typeof listOption_1.element.type !== "string") {
          return ts.filter(ts.map(value, function(v) {
            return normalizeOptionValue(listOption_1.element, basePath, v);
          }), function(v) {
            return !!v;
          });
        }
    ...
    function matchFileNames(fileNames, include, exclude, basePath, options, host, errors, extraFileExtensions, jsonSourceFile) {
      ...
      if (include && include.length > 0) {
        for (var _a = 0,
            _b = host.readDirectory(basePath, supportedExtensions, exclude, include, undefined); _a < _b.length; _a++) {
          var file = _b[_a];
    ...
    function validateSpecs(specs, errors, allowTrailingRecursion, jsonSourceFile, specKey) {
      ...
      function createDiagnostic(message, spec) {
        if (jsonSourceFile && jsonSourceFile.jsonObject) {
          for (var _i = 0,
              _a = ts.getPropertyAssignment(jsonSourceFile.jsonObject, specKey); _i < _a.length; _i++) {
            var property = _a[_i];
            if (ts.isArrayLiteralExpression(property.initializer)) {
              for (var _b = 0,
                  _c = property.initializer.elements; _b < _c.length; _b++) {
                var element = _c[_b];
    ...
    function getOptionValueWithEmptyStrings(value, option) {
      switch (option.type) {
        ...
        case "list":
          ...
          return ts.isArray(value) ? value.map(function(v) {
            return getOptionValueWithEmptyStrings(v, elementType_1);
          }) : "";
        default:
    ...
    (function(JsTyping) {
      ...
      function discoverTypings(host, log, fileNames, projectRootPath, safeList, packageNameToTypingLocation, typeAcquisition, unresolvedImports) {
        ...
        if (unresolvedImports) {
          var module_1 = ts.deduplicate(unresolvedImports.map(function(moduleId) {
            return nodeCoreModules.has(moduleId) ? "node" : moduleId;
          }));
        ...
        for (var _i = 0,
            exclude_1 = exclude; _i < exclude_1.length; _i++) {
        ...
        function getTypingNamesFromJson(jsonPath, filesToWatch) {
          ...
          var jsonConfig = ts.readConfigFile(jsonPath, function(path) {
            return host.readFile(path);
          }).config;
        ...
        function getTypingNamesFromSourceFileNames(fileNames) {
          ...
          var hasJsxFile = ts.some(fileNames, function(f) {
            return ts.fileExtensionIs(f, ".jsx");
          });
          if (hasJsxFile) {
            if (log)
        ...
        function getTypingNamesFromPackagesFolder(packagesFolderPath, filesToWatch) {
          ...
          for (var _i = 0,
            ...
            var result_2 = ts.readConfigFile(normalizedFileName, function(path) {
              return host.readFile(path);
            });
  ...
  (function(ts) {
    ...
    function resolveJavaScriptModule(moduleName, initialDir, host) {
      var _a = nodeModuleNameResolverWorker(moduleName, initialDir, {
        moduleResolution: ts.ModuleResolutionKind.NodeJs,
        allowJs: true
      }, host, undefined, true),
          resolvedModule = _a.resolvedModule,
          failedLookupLocations = _a.failedLookupLocations;
      if (!resolvedModule) {
      ...
      function tryResolve(extensions) {
        ...
        if (!ts.isExternalModuleNameRelative(moduleName)) {
          ...
          if (!compilerOptions.preserveSymlinks) {
            resolvedValue = resolvedValue && __assign({}, resolved_1.value, {
    ...
    function resolvedIfExtensionMatches(extensions, path) {
      ...
      return ext !== undefined && extensionIsOk(extensions, ext) ? {
        path: path,
        ext: ext
      } : undefined;
    }
    ...
    function loadModuleFromNodeModulesFolder(extensions, moduleName, nodeModulesFolder, nodeModulesFolderExists, failedLookupLocations, state) {
      ...
      return loadModuleFromFileNoPackageId(extensions, candidate, failedLookupLocations, !nodeModulesFolderExists, state) || loadNodeModuleFromDirectory(extensions, candidate, failedLookupLocations, !nodeModulesFolderExists, state);
    }
    ...
    function getPackageNameFromAtTypesDirectory(mangledName) {
      ...
      if (withoutAtTypePrefix !== mangledName) {
        return withoutAtTypePrefix.indexOf(mangledScopedPackageSeparator) !== -1 ? "@" + withoutAtTypePrefix.replace(mangledScopedPackageSeparator, ts.directorySeparator) : withoutAtTypePrefix;
      }
      return mangledName;
    }
      ...
      (function(typingsInstaller) {
        ...
        var TypingsInstaller = (function() {
          ...
          TypingsInstaller.prototype.install = function(req) {
            ...
            var discoverTypingsResult = ts.JsTyping.discoverTypings(this.installTypingHost, this.log.isEnabled() ? (function(s) {
              return _this.log.writeLine(s);
            }) : undefined, req.fileNames, req.projectRootPath, this.safeList, this.packageNameToTypingLocation, req.typeAcquisition, req.unresolvedImports);
            if (this.log.isEnabled()) {
        ...
        var NodeTypingsInstaller = (function(_super) {
          ...
          function NodeTypingsInstaller(globalTypingsCacheLocation, typingSafeListLocation, npmLocation, throttleLimit, log) {
            ...
            try {
              ...
              _this.execSync(_this.npmPath + " install --ignore-scripts " + TypesRegistryPackageName, {
