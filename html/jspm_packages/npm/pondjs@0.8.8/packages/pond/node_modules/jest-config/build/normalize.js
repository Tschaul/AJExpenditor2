...
const utils = require('../../jest-regex-util/build/index');
var _require2 = require('./utils');
const BULLET = _require2.BULLET,
    DOCUMENTATION_NOTE = _require2.DOCUMENTATION_NOTE,
    _replaceRootDirInPath = _require2._replaceRootDirInPath,
    _replaceRootDirTags = _require2._replaceRootDirTags,
    getTestEnvironment = _require2.getTestEnvironment,
    resolve = _require2.resolve;
var _require3 = require('./constants');
const NODE_MODULES = _require3.NODE_MODULES,
    DEFAULT_JS_PATTERN = _require3.DEFAULT_JS_PATTERN,
    DEFAULT_REPORTER_LABEL = _require3.DEFAULT_REPORTER_LABEL;
var _require4 = require('./reporterValidationErrors');
const validateReporters = _require4.validateReporters;
...
const createConfigError = (message) => new ValidationError(ERROR, message, DOCUMENTATION_NOTE);
const setupPreset = (options, optionsPreset) => {
  let preset;
  const presetPath = _replaceRootDirInPath(options.rootDir, optionsPreset);
  const presetModule = Resolver.findNodeModule(presetPath.endsWith(JSON_EXTENSION) ? presetPath : path.join(presetPath, PRESET_NAME), {basedir: options.rootDir});
  try {
  ...
  if (options.moduleNameMapper && preset.moduleNameMapper) {
    options.moduleNameMapper = Object.assign({}, preset.moduleNameMapper, options.moduleNameMapper);
  }
...
const setupBabelJest = (options) => {
  ...
  if (transform) {
    ...
    if (customJSPattern) {
      const jsTransformer = Resolver.findNodeModule(transform[customJSPattern], {basedir});
      if (jsTransformer && jsTransformer.includes(NODE_MODULES + 'babel-jest')) {
    ...
    if (babelJest) {
      options.transform = {[DEFAULT_JS_PATTERN]: 'babel-jest'};
    }
  }
  return babelJest;
};
const normalizeCollectCoverageOnlyFrom = (options, key) => {
  const collectCoverageOnlyFrom = Array.isArray(options[key]) ? options[key] : Object.keys(options[key]);
  return collectCoverageOnlyFrom.reduce((map, filePath) => {
    filePath = path.resolve(options.rootDir, _replaceRootDirInPath(options.rootDir, filePath));
    map[filePath] = true;
    return map;
  }, Object.create(null));
};
...
const normalizeUnmockedModulePathPatterns = (options, key) => {
  return options[key].map((pattern) => utils.replacePathSepForRegex(pattern.replace(/<rootDir>/g, options.rootDir)));
};
...
const normalizeMissingOptions = (options) => {
  if (!options.name) {
    options.name = crypto.createHash('md5').update(options.rootDir).digest('hex');
  }
  if (!options.setupFiles) {
    options.setupFiles = [];
  }
...
const normalizeReporters = (options, basedir) => {
  ...
  options.reporters = reporters.map((reporterConfig) => {
    const normalizedReporterConfig = typeof reporterConfig === 'string' ? [reporterConfig, {}] : reporterConfig;
    const reporterPath = _replaceRootDirInPath(options.rootDir, normalizedReporterConfig[0]);
    if (reporterPath !== DEFAULT_REPORTER_LABEL) {
      const reporter = Resolver.findNodeModule(reporterPath, {basedir: options.rootDir});
      if (!reporter) {
...
function normalize(options, argv) {
  ...
  options = normalizePreprocessor(normalizeReporters(normalizeMissingOptions(normalizeRootDir(setFromArgv(options, argv)))));
  if (options.preset) {
  ...
  Object.keys(options).reduce((newOptions, key) => {
    ...
    switch (key) {
      ...
      case 'snapshotSerializers':
        value = options[key] && options[key].map(resolve.bind(null, options.rootDir, key));
        break;
      case 'modulePaths':
      case 'roots':
        value = options[key] && options[key].map((filePath) => path.resolve(options.rootDir, _replaceRootDirInPath(options.rootDir, filePath)));
        break;
      ...
      case 'coverageDirectory':
        value = options[key] && path.resolve(options.rootDir, _replaceRootDirInPath(options.rootDir, options[key]));
        break;
      case 'moduleLoader':
      case 'resolver':
      ...
      case 'transform':
        ...
        value = transform && Object.keys(transform).map((regex) => [regex, resolve(options.rootDir, key, transform[regex])]);
        break;
      ...
      case 'haste':
        ...
        if (value.hasteImplModulePath != null) {
          value.hasteImplModulePath = resolve(options.rootDir, 'haste.hasteImplModulePath', _replaceRootDirInPath(options.rootDir, value.hasteImplModulePath));
        }
        break;
      case 'projects':
        const projects = options[key];
        let list = [];
        projects && projects.forEach((filePath) => list = list.concat(glob.sync(_replaceRootDirInPath(options.rootDir, filePath))));
        value = list;
        break;
  ...
  if (babelJest) {
    const regeneratorRuntimePath = Resolver.findNodeModule('regenerator-runtime/runtime', {basedir: options.rootDir});
    if (regeneratorRuntimePath) {
      newOptions.setupFiles.unshift(regeneratorRuntimePath);
    }
  }
  if (options.testRegex && options.testMatch) {
    throw createConfigError(`  Configuration options ${chalk.bold('testMatch')} and` + ` ${chalk.bold('testRegex')} cannot be used together.`);
  }
  if (options.testRegex && !options.testMatch) {
