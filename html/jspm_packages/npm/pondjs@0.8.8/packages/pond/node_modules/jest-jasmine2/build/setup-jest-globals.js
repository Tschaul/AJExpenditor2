/* */ 
'use strict';
var _require = require('../../jest-matchers/build/index');
const getState = _require.getState,
    setState = _require.setState;
var _require2 = require('../../jest-snapshot/build/index');
...
function addAssertionErrors(result) {
  ...
  if (typeof expectedAssertionsNumber === 'number' && assertionCalls !== expectedAssertionsNumber) {
    const expected = EXPECTED_COLOR(pluralize('assertion', expectedAssertionsNumber));
    const message = new Error(matcherHint('.assertions', '', String(expectedAssertionsNumber), {isDirectExpectCall: true}) + '\n\n' + `Expected ${expected} to be called but only received ` + RECEIVED_COLOR(pluralize('assertion call', assertionCalls || 0)) + '.').stack;
    result.status = 'failed';
  ...
  if (isExpectingAssertions && assertionCalls === 0) {
    ...
    const message = new Error(matcherHint('.hasAssertions', '', '', {isDirectExpectCall: true}) + '\n\n' + `Expected ${expected} to be called but ${received}.`).stack;
    result.status = 'failed';
    result.failedExpectations.push({
      actual: 'none',
