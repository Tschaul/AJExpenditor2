...
(function(process) {
  ...
  var _require2 = require('../../jest-haste-map/build/index');
  const getCacheFilePath = _require2.getCacheFilePath;
  const stableStringify = require('../../json-stable-stringify/index');
  ...
  class ScriptTransformer {
    ...
    _getCacheKey(fileData, filename, instrument, mapCoverage) {
      ...
      if (transformer && typeof transformer.getCacheKey === 'function') {
        return transformer.getCacheKey(fileData, filename, configString, {instrument});
      } else {
        return crypto.createHash('md5').update(fileData).update(configString).update(instrument ? 'instrument' : '').update(mapCoverage ? 'mapCoverage' : '').digest('hex');
      }
    }
    _getFileCachePath(filename, content, instrument, mapCoverage) {
      const baseCacheDir = getCacheFilePath(this._config.cacheDirectory, 'jest-transform-cache-' + this._config.name, VERSION);
      const cacheKey = this._getCacheKey(content, filename, instrument, mapCoverage);
      const cacheDir = path.join(baseCacheDir, cacheKey[0] + cacheKey[1]);
      const cachePath = slash(path.join(cacheDir, path.basename(filename, path.extname(filename)) + '_' + cacheKey));
    ...
    _instrumentFile(filename, content) {
      ...
      const babelPluginIstanbul = require('../../babel-plugin-istanbul/lib/index').default;
    ...
    transformSource(filename, content, instrument, mapCoverage) {
      ...
      const cacheFilePath = this._getFileCachePath(filename, content, instrument, mapCoverage);
      let sourceMapPath = cacheFilePath + '.map';
      let code = this._config.cache ? readCacheFile(filename, cacheFilePath) : null;
      if (code) {
        return {
          code,
          sourceMapPath
      ...
      if (transform && shouldTransform(filename, this._config)) {
        const processed = transform.process(content, filename, this._config, {instrument});
      ...
      if (mapCoverage) {
        if (!transformed.map) {
          const convert = require('../../convert-source-map/index');
    ...
    _transformAndBuildScript(filename, options, instrument, fileSource) {
      ...
      const willTransform = !isInternalModule && (shouldTransform(filename, this._config) || instrument);
      try {
        if (willTransform) {
          const transformedSource = this.transformSource(filename, content, instrument, !!(options && options.mapCoverage));
    ...
    transform(filename, options, fileSource) {
      ...
      const scriptCacheKey = getScriptCacheKey(filename, this._config, instrument);
  ...
  const shouldTransform = (filename, config) => {
    ...
    return (!!config.transform && !!config.transform.length && (!config.transformIgnorePatterns.length || !isIgnored));
  };
