...
(function(Buffer, process) {
  ...
  function install(gyp, argv, callback) {
      ...
      mkdir(devDir, function(err, created) {
        ...
        req.on('error', function(err) {
          if (err.code === 'ENOTFOUND') {
            return cb(new Error('This is most likely not a problem with node-ninja or the package itself and\n' + 'is related to network connectivity. In most cases you are behind a proxy or have bad \n' + 'network settings.'));
          }
          badDownload = true;
          cb(err);
        });
    ...
    function eaccesFallback() {
      ...
      gyp.devDir = path.resolve(tmpdir, '.node-ninja');
  ...
  function download(gyp, env, url) {
    ...
    var requestOpts = {
      uri: url,
      headers: {'User-Agent': 'node-ninja v' + gyp.version + ' (node ' + process.version + ')'}
    };
