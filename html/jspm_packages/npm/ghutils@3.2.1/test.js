...
test('that lister follows res.headers.link', function(t) {
  ...
  var auth = {
    ...
    token: 'authtoken'
      ...
      server;
  server = util.makeServer(testData).on('ready', function() {
    var result = testData[0].response.concat(testData[1].response);
    ghutils.lister(xtend(auth), urlBase, {}, util.verifyData(t, result));
  }).on('request', util.verifyRequest(t, auth)).on('get', util.verifyUrl(t, ['https://api.github.com/foobar', 'https://somenexturl', 'https://somenexturl2'])).on('close', util.verifyClose(t));
});
test('test list multi-page pulls, options.afterDate includes all', function(t) {
  ...
  var auth = {
    ...
    token: 'authtoken'
      ...
      testData = [{
        response: [{
          test1: 'data1',
          created_at: new Date('2015-12-14T05:58:14.421Z').toISOString()
        }, {
          test2: 'data2',
          created_at: new Date('2015-12-13T05:58:14.421Z').toISOString()
        }],
        headers: {link: '<https://api.github.com/foobar?page=2>; rel="next"'}
      }, {
        response: [{
          test1: 'data3',
          created_at: new Date('2015-12-12T05:58:14.421Z').toISOString()
        }, {
          test2: 'data4',
          created_at: new Date('2015-12-11T05:58:14.421Z').toISOString()
        }],
  ...
  }).on('request', util.verifyRequest(t, auth)).on('get', util.verifyUrl(t, ['https://api.github.com/foobar', 'https://api.github.com/foobar?page=2', 'https://api.github.com/foobar?page=3'])).on('close', util.verifyClose(t));
});
test('test list multi-page pulls, options.afterDate includes all', function(t) {
  t.plan(10);
  var auth = {
  ...
  }).on('request', util.verifyRequest(t, auth)).on('get', util.verifyUrl(t, ['https://api.github.com/foobar', 'https://api.github.com/foobar?page=2'])).on('close', util.verifyClose(t));
});
test('valid response with null data calls back with null data', function(t) {
  ...
  }).on('request', util.verifyRequest(t, auth)).on('close', util.verifyClose(t));
});
