...
const {LENGTH,
  PERCENT,
  SPACE} = tokens;
module.exports.directionFactory = ({types = [LENGTH, PERCENT],
...
module.exports.anyOrderFactory = (properties, delim = SPACE) => (tokenStream) => {
  ...
  while (numParsed < propertyNames.length && tokenStream.hasTokens()) {
    ...
    const matchedPropertyName = propertyNames.find((propertyName) => (values[propertyName] === undefined && tokenStream.matches(properties[propertyName].token)));
