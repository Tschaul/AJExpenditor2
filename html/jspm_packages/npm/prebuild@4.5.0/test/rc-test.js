...
(function(process) {
  ...
  test('custom config and aliases', function(t) {
    var args = ['--prebuild vX.Y.Z', '--prebuild vZ.Y.X', '--arch ARCH', '--platform PLATFORM', '--download https://foo.bar', '--upload t00k3n', '--compile', '--debug', '--force', '--version', '--help', '--path ../some/other/path', '--preinstall somescript.js'];
  ...
  test('using --all will build for all targets', function(t) {
    var args = ['--prebuild vX.Y.Z', '--prebuild vZ.Y.X', '--all'];
    runRc(t, args.join(' '), {}, function(rc) {
  ...
  test('npm args are passed on from npm environment into rc', function(t) {
    var env = {npm_config_argv: JSON.stringify({cooked: ['--compile', '--build-from-source', '--debug']})};
  ...
  test('modules are build from source when used inside electron', function(t) {
    var env = {npm_config_runtime: 'electron'};
    runRc(t, '', env, function(rc) {
      t.equal(rc.compile, true, 'compile is set');
      t.end();
    });
