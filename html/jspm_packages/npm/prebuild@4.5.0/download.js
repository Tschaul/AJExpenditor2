  ...
  function downloadPrebuild(opts, cb) {
    ...
    function unpack() {
      ...
      var updateName = opts.updateName || function(entry) {
        if (/\.node$/i.test(entry.name))
          binaryName = entry.name;
      };
      log.info('unpacking @', cachedPrebuild);
      pump(fs.createReadStream(cachedPrebuild), zlib.createGunzip(), tfs.extract(opts.path, {
        readable: true,
        writable: true
      }).on('entry', updateName), function(err) {
        if (err)
          return cb(err);
        if (!binaryName)
          return cb(error.invalidArchive());
        var resolved;
        try {
